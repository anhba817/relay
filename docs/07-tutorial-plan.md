...
### Part 3 — Becoming a platform (26 chapters, eight movements)
...
**AND A FOURTH TIME, WHICH IS WHAT THIS PARAGRAPH IS FOR.** Feature 045 regrouped Part 3 into
eight contiguous movements and renumbered it to 26 — two published chapters split, so 24 + 2 —
and left the heading at 24 and the summary table at the top of this document at **21**, a number
already two corrections stale. Three places, three different counts, all of them written by
somebody who had read the sentence above. The count is now stated as 26 in both places and
derived in the row underneath: **the `Ch` column is the tree, and `check-map` compares it to
`chapter-map.json` rather than to this heading.**

**THE `Built` COLUMN IS THE PUBLISHED RECORD AND SPEAKS THE PUBLISHED NUMBERING.** Each
description was written when its chapter shipped and is kept verbatim, so an ordinal inside one
— "the transport 3.6 was owed", "the membership graph 3.15 and 3.16 built" — means the chapter
in the `Was` column of the row that carries that number, not the `Ch` column. Rewriting those
ten citations to the new numbers would have made this table disagree with the twenty-four
close-out records that use the old ones. Two rows say `(half)` in `Was` because one published
chapter became two: 3.12 into new 4 and new 25, 3.14 into new 3 and new 26.

| Ch | Title | Built |
|---|---|---|
| Ch | Was | Movement | Title | Built |
|---|---|---|---|---|
| 1 | 3.1 | I | Tenants all the way down | Orgs, apps, environments; OAuth signup; auto-created dev environment (FR-TEN-02) |
| 2 | 3.2 | I | Keys and tokens | API keys (prefix, hash, rotation); user JWTs; the dev-token endpoint (FR-AUT-09); error messages that name the wrong-credential mistake |
| 3 | 3.14 (half) | I | Errors that resolve | The error half of published 3.14, moved to the front because eleven chapters after it raise `protocolError` and the vocabulary has to exist before the first of them: one registry, one URL rule, a typed thrower, and `docs_url` resolving against the published reference. The outsider half stays at the end as new 26 — the two share no fence |
| 4 | 3.12 (half) | I | The isolation harness | The harness half of published 3.12, moved ahead of every chapter that uses it. `packages/test-harness`, the global-operation guard, and the structural check that every table has a path back to a tenant. Feature 030 built these inside published 3.10's span with no chapter of its own, which is why three chapters excerpted these files and none of them built them; the gauntlet that attacks with the harness stays at the end as new 25 |
| 5 | 3.3 | II | The outbox | Transactional outbox + relay (ADR-06); the dual-write problem demonstrated with a crash-in-the-gap test |
| 6 | 3.4 | II | JetStream and the first consumer | NATS setup; subjects; durable pull consumers |
| 7 | 3.7 | II | Commit and publish are two instants | The resume duplicate: a message committed before a backfill and announced after it, delivered twice; the high-water mark given a lifetime past the buffering window |
| 8 | 3.13 | III | The endpoints and the instruments | The two public endpoints Part 3 needed and nobody had built — `POST /v1/channels` and its members route — with idempotency enforced by a unique index rather than in application memory; every validation error naming its field for the first time since EIR-API-06 asked; the global-operation guard extended from five tables to nine; and the api repository layer's branch coverage answered with a number rather than a restatement |
| 9 | 3.15 | III | The channel a customer controls | FR-CHN-03/04/05/06/10: the `private` type made to decide something on all four of its doors, bulk member removal, member roles with their own vocabulary, and archiving that refuses a send without announcing the channel exists. 20 files, 2,947 prose words, 20 fences, 3 figures |
| 10 | 3.16 | III | What a user sees | FR-CHN-08/09 and all of FR-USR: channel listing with cursor pagination and activity ordering, unread counts derived from a sequence the write path already maintains, user profiles, bulk upsert, a deletion that keeps the row, and banning. 24 files |
| 11 | 3.17 | III | The sender a message never had | A message sent by a customer's server has no sender at all — chapter 3.3 decided that when nothing read one. Bot users with descriptions, a sender required on every send, and an application credential that may speak as software and not as any person. Amends the SRS (FR-USR-07 and FR-MSG-15 added, FR-MSG-13 and FR-RTL-05 narrowed in place) and the SAD's `users` DDL |
| 12 | 3.18 | IV | The message that never arrived | **FR-RTM-01**, not FR-RTM-05, which this row cited from 3.14 to 3.18: a connected client shall receive messages for every channel of which it is a member, and the api never satisfied it. The api publishes to `chan:{channel_id}` after commit, so a REST-sent message reaches a socket. Closes chapter 3.12's `gaps.md` G1 — both mechanisms, the sender in 3.17 and the publish here — and the concrete half of chapter 3.14's Phase 2 verdict: an outsider who sent over REST and waited on a socket could not succeed, and no document said so |
| 13 | 3.19 | IV | Presence, and who is allowed to see it | **FR-RTM-05**'s presence half, **FR-RTM-06**'s online/offline with a 30-second grace period, and **FR-RTM-07**'s scoping — reaching only users who share a channel with the subject, which is the membership graph 3.15 and 3.16 built. `presence.changed` had been in the protocol union since 1.3 with nothing producing one. The scoping needs no filtering code: it falls out of a second subject grammar, `presence:{channel_id}`, which supersedes ADR-10's subject clause as **ADR-19** because `chan:{id}` is typed to messages at three points. Completes **FR-CHN-05**'s third verb — *observe presence* — and closes SRS open question 3 as **not** opt-in per channel. Three separate 30-second numbers turned out to be three quantities, and a TTL equal to its own refresh interval expires a connected user |
| 14 | 3.20 | IV | The membership that changed under a live socket | **FR-RTM-10**'s five-second revocation, unmet since 2.6 and asserted as VIOLATED by a test in `session.itest.ts` since 3.18 — that test is inverted here with its own 5,500 ms wait unchanged. Completes **FR-RTM-05**'s third event kind, `membership.changed`, and gives **FR-WHK-02**'s `channel.member_added` and `channel.member_removed` their first producers. A **third subject grammar with two shapes** (**ADR-20**): a removal rides `member:{channel_id}` and reaches both audiences in one publish because the removed user is still a member at that instant, while an addition cannot — so `member:{env}:{user}` exists, and it is the first event in this system addressed to a **principal** rather than a channel. ADR-07 permits a lossy fabric because a dropped message is recovered by its cursor; **a revocation has no cursor**, so a sixty-second re-read is what stands in for one. Two tasks specified an ordering as the requirement and neither ordering turned out to be observable — both found by running the proof the task itself asked for |
| 15 | 3.21 | IV | The frame nobody may send | **FR-RTM-08** closed, and FR-RTM-05's sixth frame kind given a producer. The chapter's own plan said typing was the one kind that could reuse `chan:{channel_id}`; re-deriving ADR-19's count found seven typed points where the record says three, so it takes a fourth subject grammar and says why. **The first chapter to open a second inbound frame** — `session.ts` refused everything but `message.send` for twenty chapters — and the first requirement this platform cannot keep: no frame ends an indicator, so the five-second expiry belongs to the receiving client, recorded in ADR-22 rather than asserted as closed |
| 16 | 3.22 | IV | The sixth connection, and where the count lives | **FR-RTM-09**, both halves and they were in different states. The cap — five concurrent connections per user — was enforced nowhere, and the clause's second half, *"each shall receive all events independently"*, has been true since delivery walked connections rather than users; this chapter gives it tests rather than behaviour. **The count cannot live in an instance** (CON-02), so it lives in Redis as five slot keys claimed with `SET NX PX` — which **contradicts `docs/05-sad.md`'s §6.3 row**, published since the first draft, prescribing a sorted set pruned with `ZREMRANGEBYSCORE`. That shape needs Lua to make the claim atomic and Constitution VII will not admit a second language without profiling evidence a five-channel fixture cannot produce, so the chapter argues the row down in **ADR-23**. A **sixth close code**, 4004, because all five existing ones send a client to the wrong remedy — this is the only refusal in the set whose correct handling is not a retry. The refusal completes the handshake in order to close it: a browser cannot read the body of a failed upgrade |
| 17 | 3.23 | V | The words somebody wants back | **FR-MSG-07, FR-MSG-08 and FR-MSG-10**, and three of the four design decisions were already written in the tree — the refusal seam, the close-code test, the intervals — so the chapter's work was reading rather than deriving. `message_edits` had been published in `docs/05-sad.md` §6.1 since the first draft and never built; `messages.edited_at`, `deleted_at` and `metadata` had been in the schema, unwritten, since chapter 2.1. **A tombstone is not a `Message`** — it has no text — so a deletion cannot ride `chan:{channel_id}`, and an edit IS one and would be indistinguishable from a creation on it. The chapter takes a **fifth subject grammar**, `revision:{channel_id}` with the kind in the payload, as **ADR-24**: a kind that cannot share a payload type cannot share a subject, which three prior chapters reached independently. **Two new error codes**, because `forbidden`'s published remedy — *a change of credential or of permission* — is advice nobody can act on when the fact is authorship. The one soft edge is documented rather than closed: **a message older than a client's cursor that changed during a disconnect produces no frame and no sequence gap**, so the mechanism that repairs every other missed frame sees nothing to repair, and history is the repair. That is what Slack does; Matrix's append-only timeline and IMAP's `MODSEQ` are the two alternatives, and both add a per-channel counter every mutation has to maintain |
| 18 | 3.24 | V | The message that is not only text | **FR-MSG-11's external-URL half**, and the chapter is mostly about what a required field is worth. Making `messageSchema.attachments` required rather than optional made the compiler name four production sites and 29 in tests — and the three it could not name are the ones that cost the most, because a value handed to `parse` is `unknown` no matter what the schema says. The `media_id` half is refused by the SCHEMA rather than a controller, carrying its own code and status in the issue, because a controller check runs after the validation pipe has already thrown. |
| 19 | 3.5 | VI | Webhooks that survive the customer | Dispatcher service: envelope-encrypted signing secrets, HMAC-SHA256, a due-time retry schedule, dead letters (FR-WHK-01…05, FR-WHK-08) |
| 20 | 3.6 | VI | When to stop trying | The attempt log and auto-disable (FR-WHK-06, FR-WHK-07, both half-delivered — see below); the synthetic test event (FR-WHK-09); the evidence a customer is owed when their endpoint is switched off |
| 21 | 3.9 | VI | The email nobody was sending | The transport 3.6 was owed (FR-WHK-07): the outbox pattern a third time, over `webhook_disable_notifications` — no migration, because `delivered_at` was already there and already null. Mailpit in compose, and tests that read what was **received** |
| 22 | 3.8 | VII | Limits you can see coming | Per-environment fixed-window counters in Redis (FR-RTL-01…04); the three headers on every response, not only the refusal; failed-auth limiting per IP (FR-AUT-12), which fails **closed** while the tenant limiter fails open. **This chapter completes SRS Phase 2's requirement set** — §7.3 lists it as FR-TEN, FR-AUT, FR-WHK and FR-RTL at P2, and FR-RTL-01…04 is the last of the four |
| 23 | 3.10 | VII | Quotas and what they cost | Monthly usage quotas for messages and distinct active users, a hard cap that refuses sends with `402` and a soft threshold that only alerts, the 50/80/100% email (FR-RTL-05…08). The caps live in `environments.quota_config` — the jsonb column 2.1 declared and 3.8 refused in print. **The outbox pattern a fourth time**, and the first chapter in the series that needed no global operation at all: usage rises only on a send, so the send knows what it crossed |
| 24 | 3.11 | VII | Counting a connection | Connection-minutes (the third dimension of FR-RTL-05): periodic accounting in the gateway, which owns no tables and — until this chapter — no identity either; reports that carry totals so a lost one repairs itself; the crash that under-bills by a bounded amount rather than over-billing for ever; and close code 4008, declared in 1.3 and emitted for the first time here |
| 25 | 3.12 (half) | VIII | **Milestone: the isolation gauntlet** | The cross-tenant attack suite (NFR-SEC-09), with a target list derived from the running router rather than maintained by hand; four attack shapes over 24 routes; a structural check that every table has a path back to a tenant; the socket surface attacked from the protocol's own frame union; and three deliberate reintroductions, one of which stayed green and taught the suite's range |
| 26 | 3.14 (half) | VIII | **Milestone: an outsider integrates** | Thirteen error codes with one registry and one URL rule, a `docs_url` that resolves against the published reference, and a sealed integration package mechanically unable to import workspace code. This chapter gives the SRS Phase 2 exit criterion its verdict — *"an external developer integrates using only public documentation, with no assistance"* |
...
| 2 | II | ClickHouse from zero | MergeTree, `PARTITION BY` (DR-07), `ORDER BY (environment_id, ts)` — the two columns chapter 1 showed Postgres orders by neither of — TTL (DR-09) |
| 3 | II | A second store needs a second ledger | A migration runner and an identity scheme for a store the Postgres runner cannot serve. Open; everything downstream anchors on it |
| 4 | II | The consumer that was promised | The ingester. Batching (DR-11), backpressure, ClickHouse down → the stream absorbs 24 h (NFR-REL-05) |
| 5 | III | Every request is an event | FR-ANL-07's producer. **Generalises rather than introduces**: chapter 3.20 already shipped `analytics.{domain}.{action}.{env}` and argued the fire-and-forget tradeoff in full |
| 6 | III | The gateway's first stream | Connection open/close (FR-ANL-01). The gateway has never touched NATS; amends ADR-07 a second time |
| 7 | IV | Metering you can bill on | Daily rollup MVs (DR-10) — billing never scans raw events |
...
| 14 | VI | The only service that reads the bytes | The media worker (ADR-14). FR-MED-03/04: verify against declaration, ClamAV, probe. **Shipped at feature 059**: dimensions, not duration (FR-MED-04 partly met, SRS 1.20); ADR-31 argues the fifth service against SAD §4.2 and ADR-32 the language, closing `docs/12` §7.3 |
...
| 18 | VI | **Milestone: an image, end to end** | Upload → scan → send → signed delivery. FR-MED-09's rejection renders as rejected, never as broken |
| 19 | VII | The log that cannot be edited | FR-MOD-03's audit log. **First in its movement, not last** — everything after writes to it, which makes it registry-shaped, the same shape as Part 3's *Errors that resolve* |
| 20 | VII | Everything, including what was deleted | FR-MOD-01/02 via API key. Both are **P2**, and chapter 3.23 built edit history and tombstones — run the premise before writing it |
