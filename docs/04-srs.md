...
| EIR-WS-03 | The server shall send a `connection.ack` frame containing the resolved user identity, a resume cursor, and a per-channel revision count within 1 second of a successful handshake. The revision count is reported for every channel of which the user is a member, including those at zero, so that a client with nothing to repair still receives the baseline it will compare against at its next reconnection (feature 044). | P1 | T |
...
| FR-RTM-03 | On reconnection with a resume cursor, the system shall deliver all messages with a sequence number greater than the cursor for every channel of which the user is a member. A message **revised** below the cursor is not delivered and produces no sequence gap, because an edit or a deletion carries the sequence of the message it changes rather than a new one; the system shall instead report a per-channel count of revisions in `connection.ack` (EIR-WS-03), from which a client determines which channels to repair by re-reading history. The system shall not perform that comparison and shall not refuse a connection on the basis of a count a client presents (feature 044). | P1 | T |
...
| FR-RTM-05 | The system shall emit real-time events for message creation, edit, deletion, membership change, presence change, and typing. Edit and deletion events reach only clients connected when they occur; a client that was disconnected learns of them from the revision count in `connection.ack` (FR-RTM-03) rather than from a replayed event. | P1 | T |
...
| FR-RTM-09 | A user shall be permitted up to 5 concurrent connections; each shall receive all events independently. The cap is **best-effort**: it is enforced from a connection registry that is not a source of truth, and when that registry is unreachable the connection is admitted and the gateway records that the cap was not checked. A user may therefore exceed 5 concurrent connections while the registry is unavailable (ADR-23). | P2 | T |
| FR-RTM-10 | Events shall not be delivered to a client whose membership no longer grants access, effective within 5 seconds of the membership change. When the membership fabric is unavailable the five-second bound does not hold and revocation is carried by a per-connection re-read every 60 seconds, so a revocation lands **within 60 seconds** — exceeding the five-second bound by 55. Revocation remains guaranteed; what is bounded is how late it can be (ADR-20). | P1 | T |
...
| FR-ANL-05 | The system shall meter, per tenant per day: messages sent, unique active users, connection-minutes, and stored message count. **A connection-minute is a calendar minute during any part of which a connection was open** (Appendix C question 4, closed in v1.13). | P3 | T |
| FR-ANL-06 | Metered totals shall agree with counts derived from operational data to within 0.1%, verified by a daily reconciliation job that raises an alert on breach. **The comparison is per tenant and per period; an aggregate one reports a number nobody would question.** Measured at chapter 4.7: the two operational counters of messages sent disagree by **0.2630%** across the lane while **49 of 1,385 tenant-periods** breach the bound and one is wrong by everything it has. **"Counts derived from operational data" is not one number, so the report names the table it read**: `usage_periods` for messages sent and connection-minutes, `usage_active_users` for unique active users, and **nothing at all for stored message count**, which has no operational counterpart in this platform. **A quantity with no operational source is `not-comparable` and a tenant with nothing on either side is `no-data`; neither is a breach and neither is agreement.** At chapter 4.7 every tenant in the platform was in one of those two states — 4 environment ids in the rollup, none of which exist in Postgres, against 1,313 with operational usage, none of which have rollup rows. **AND THE 0.1% BOUND IS UNREACHABLE FOR THREE OF THE FOUR QUANTITIES, FOR REASONS THAT ARE NOT DEFECTS.** Unique active users: `uniq` is exact to **65,536** distinct and off by **0.5676%** at 65,537 (`gaps.md` 047-1, re-measured at 4.7). Any quantity read at the raw retention boundary: a daily rollup's finest grain is a day and the TTL cuts at a timestamp, so the oldest day in the window disagrees by **0% at midnight rising to 1.0989% just before it**, on the same data. Connection-minutes: the meter bills every calendar minute a connection was open and the records bill only connections that **closed**, and `wss.close()` leaving established sockets to the process exit is chapter 4.5's decision rather than its defect. **AND "RAISES AN ALERT" HAS NO MECHANISM HERE.** This platform has no alerting integration; the job exits non-zero, and the two mail paths that exist (`quotas/quota-email.ts`, `notifications/notification-relay.ts`) share one transport whose default is a local catcher. An exit code is what the clause can currently mean. **VERIFICATION STATUS, 2026-09-17 (feature 054, chapter 4.9), AND IT COVERS ONE OF THE CLAUSE'S THREE PARTS.** The letter is `T`, so the discharge is a test and not a document: `services/api/src/metering/reconcile.itest.ts` plants a drift at the bound and one unit past it and runs on every push. **The comparison is verified.** The *daily job* is not — nothing invokes `scripts/reconcile-usage.mjs`: zero occurrences in either repository's `package.json`, in `turbo.json`, in `ci.yml` or in any shell script, and `ci.yml` has no `schedule:` trigger (ADR-28, which records the absence rather than building a sixth background relay that would report `no-data` for every tenant on the platform). The *alert* is not, for the reason two sentences above. **And the bound's resolution is measured rather than asserted**: `docs/13-metering-measurement-2026-09-17.md` publishes 121,057 against 121,057 — **0.0000%** — in one tenant-period where the smallest expressible drift is **122 messages, 0.1008%**, with a planted 121 passing and 122 breaching in both directions. That document is **evidence about the bound, not the verification of the clause**; `docs/11` could be the verification for NFR-SCL-01 only because that clause's letter is `A`. | P3 | T |
| FR-ANL-07 | The system shall retain a queryable API request log per tenant for 30 days, recording request ID, timestamp, endpoint, method, status, and latency. Request and response bodies shall not be recorded. | P3 | T |
| FR-ANL-08 | Analytical queries over 90 days of a single tenant's data shall return within 2 seconds at p95. **Over `api_requests` the ninety days cannot exist**, and that is a property of the schema rather than of any lane's size (feature 053, chapter 4.8). FR-ANL-07 retains that table for **30 days** and `analytics/0005_connection_events.sql:4` records why one clause cannot express two; measured at 4.8, two rows planted for one environment at `now - 60 days` and `now - 1 day` leave **one survivor**, and `OPTIMIZE TABLE … FINAL` changes nothing — the TTL removes rows **at INSERT** (chapter 4.2's finding, reproduced). **So no fixture can put a 90-day window in front of this clause**, at any volume. Where the two are read over the same table, **FR-ANL-07's retention governs and this clause is measured over whatever the retention allows**, with the window stated beside the figure. The clause is unchanged for the tables DR-09 retains for 90 days, which is what it was written for. | P3 | A |
| FR-ANL-09 | Usage shall be attributable by application, environment, channel, and day. | P3 | T |
| FR-ANL-10 | The system shall compute end-to-end delivery latency percentiles (p50, p95, p99) per tenant per hour, **computed with `quantileExact` and never with `quantile`**. **"End-to-end delivery latency" is the interval from a message's commit to the frame being written to a subscriber's socket** — the real-time path constitution II is about, and the reading `message_events.delivery_latency_ms` was created for. `analytics/0003_webhook_attempts.sql:20` has said so since it was written: that column is *"how long the ENDPOINT took to answer. NOT `message_events.delivery_latency_ms`, which is how long a message took to reach a client."* **The two readings not chosen, and what each would cost.** *Webhook delivery* — commit to the endpoint answering — is the last leg only: `services/dispatcher/src/deliver.ts` sets `started` immediately before the `fetch` and takes the difference immediately after, so it excludes the commit, the outbox, JetStream, the dispatcher's claim and every retry gap; it already has a column and a name of its own, and relabelling it would publish a number under a name meaning something else. *A REST history read* has no delivery in it at all — a read nobody made has no latency — and it is pull rather than push. **AND THE CHOSEN READING HAS NO PRODUCER, WHICH IS A COST AND NOT AN OVERSIGHT.** Measured at chapter 4.8: `message_events` holds **0 rows** and **0 of its rows carry a latency**; the three occurrences of the table name under `services/` are a comment, a cleanup and an insert, all belonging to one test (`gaps.md` 048-2, carried through six features and re-measured here). The one thing that writes the table at all, `scripts/scale/load-analytics.mjs`, writes `CAST(NULL AS Nullable(UInt32))` into that column twice, on purpose. Building it needs both instants carried across a service boundary — the commit is the api's and the socket write is the gateway's — on the busiest path in the platform, which is the same cost chapter 4.6 weighed and deferred for a different column. **AND `quantile` HAS NO EXACT REGIME, WHICH IS THE OPPOSITE SHAPE FROM `uniq`.** `uniq` is exact below 65,536 and wrong above; `quantile` is approximate everywhere and **the error does not fall as the sample grows**. Measured against the server at chapter 4.8, over a uniform sample the p50 error is **16.67% at n=4**, reaches 0 at n=155, and is back to **0.90% at n=10,000**; over a skewed sample — which is what a latency is — it **grows** with the sample, reaching **9.52% on all three percentiles at n=10,000**. On the one real latency sample this platform holds, `webhook_attempts` at n=64, `quantile(0.99)` answers **4,961.26** where `quantileExact(0.99)` answers **10,000** — **50.39% low**, and low is the direction that matters, because an alert threshold set on it never fires. **AND THE BUCKET THIS CLAUSE ASKS FOR IS THE SMALL ONE.** Per tenant per hour over the lane's request log: **178 buckets, median 10 rows, p95 155, max 171**, and **73 of the 178 hold fewer than five rows**. A p99 over four samples is a maximum wearing a percentile's name, so any implementation publishes `n` beside every percentile. | P3 | T |
...
| FR-DSH-03 | The dashboard shall provide a searchable request log filterable by endpoint, status, and time range. **"With full request and response detail" is struck** (feature 053, chapter 4.8): FR-ANL-07 was amended at revision 1.11 to record no bodies at all, and EIR-DSH-02 permits the dashboard *"only the same public API available to customers"* — so the dashboard can show only what that API returns, and that API returns no bodies. **The three clauses could not all hold**, and the two that reach further are the ones that stayed: FR-ANL-11 keeps message content out of the analytical store and constitution VI keeps it out of logs. 1.11's own sentence is the loss, unchanged and now carried to the clause it also governs: *a request log with no payload cannot answer "what exactly did they send", only "what did they call and what happened"*. **What the dashboard can show is exactly the surface chapter 4.8 built** — `GET /v1/request-log`, with the endpoint, status and time-range filters this clause names and a page envelope conforming to EIR-API-06. | P3 | D |
...
| DR-09 | Raw events shall be retained for 90 days; daily aggregates for 25 months. **The second half went unimplemented for three features and was filed as absent twice** — `gaps.md` 047-3 and 048-4 both read *"no clause says how long metering history is kept"* while quoting DR-10, one row below this one. Chapter 4.6 applied it: `TTL toDateTime(day) + INTERVAL 25 MONTH` on both rollups. It does not contradict chapter 4.2's decision that metering must not lose history when raw events do (`specs/047-chapter-4-2/spec.md` FR-003a — a feature-local id, not a clause in this document, and cited here as one until v1.14) — 25 months against 90 days outlives the source roughly eightfold, and *"not the raw table's TTL"* was all that decision ever asked for. |
| DR-10 | Materialised views shall maintain daily per-tenant rollups for metering, so billing never scans raw events. **A rollup satisfies this clause only if it is both read and reachable from the data**: chapter 4.6 measured a conforming view over a source with no producer, read by nothing, and a second whose key carried an attribution dimension and made the billing read touch more rows than the raw table it replaced (147,534 rows against 281; 32,778 read against 32,768). **Attribution and billing take separate rollups.** And a materialised view is a trigger on future inserts, so any deployment applying these statements to a store that already holds data needs a backfill — which recovers only what the raw retention has not yet removed. |
| DR-11 | Inserts shall be batched or use asynchronous insert mode; single-row synchronous inserts are prohibited. **The ingester's bounds are 2 seconds or 10,000 rows, whichever comes first, and it takes BOTH because each fails alone: a row count never flushes for a quiet tenant, and an interval has no ceiling under load. Measured: the two cross at 5,000 records/second.** Added in revision 1.12 — `docs/05-sad.md` §4 had carried those two numbers since its first draft and attributed them here, and this clause named neither. |
...
| ~~4~~ **Closed in v1.13** | Does connection-minute metering need per-second precision, or is per-minute rounding acceptable? **Per-minute rounding, and the minute is a CALENDAR bucket rather than a rounded duration.** A connection is charged for every calendar minute it was open for any part of: 00:00:59 to 00:01:01 is two seconds of wall clock and two connection-minutes. That is what `meter.ts` has billed since chapter 3.24 and what chapter 4.6's rollup computes, so the two agree by construction rather than by reconciliation. **Measured over 55 real connections: 56 calendar minutes against 0.6513 elapsed minutes, a factor of 86**, because the median connection lasted 252 ms and each one occupies a whole minute. The alternative — bill elapsed duration, which is literally what a connection record contains — was rejected on that number: a client reconnecting every quarter-second would consume a connection slot continuously and pay almost nothing, and the cost a connection imposes is a handshake, a slot claim and a Redis round trip rather than the seconds it then sits idle. **What is given up is stated**: the calendar bucket over-reports short connections against wall-clock intuition, and a customer comparing their own timings against an invoice will find the invoice larger. | FR-ANL-05 | Product / Billing |
...
| 1.6 | 2026-09-03 | — | **FR-MSG-07, FR-MSG-08 and FR-MSG-10 built** (chapter 3.23). No clause changed. Three consequences worth recording: **FR-RTM-05's six kinds all have producers now** — message edit and message deletion had none anywhere in the platform, and a test reads `session.ts` as text to keep that true, because nothing else can see a producer. **FR-WHK-02 goes from three of eight event types to five**, `message.updated` and `message.deleted` spelled as that clause spells them. And **FR-MSG-08's "deletion metadata"** is read as more than `deleted_at`, because the clause itemises timestamps separately — the deletion's actor is recorded in `messages.metadata` as `deleted_by`, needing no migration. Recorded as ADR-24: message revisions take a fifth subject grammar, `revision:{channel_id}`, because a kind that cannot share a payload type cannot share a subject. `message_edits` is built to §6.1 as published — three columns and a composite key. Two new error codes, `not_message_author` and `message_deleted`, both argued against the generic 403 in `codes.ts`. The one soft edge is documented rather than closed: resume is ordered by the channel sequence alone, so a message older than a client's cursor that changed during a disconnect produces no frame **and no sequence gap**, and history is the repair — which is what Slack does |
| 1.7 | 2026-09-04 | — | **FR-MSG-11's external-URL half built** (chapter 3.24). No clause changed. The clause publishes two ways to attach and one is built: a `{"type": "url", "kind", "url"}` attachment is accepted, bounded at ten per message and 2,048 characters per URL, and refused unless its scheme is `http` or `https`. **The `media_id` half is deferred to §4.14 and refused by name**: a new error code, `media_not_available`, 422, because `media_id` is in the published contract and a caller sending one has made no mistake — `invalid_request` would send them to check their JSON. The shape is a discriminated union from its first version, so §4.14 replaces that arm's body rather than introducing a discriminator. Three consequences worth recording: **a message may now carry no text** when it carries an attachment, storing `text = ""` rather than a null so the tombstone predicate is untouched; **the same URL twice is two attachments**, because the platform does not compare them; and **no bytes cross Relay compute**, which is §4.14's premise and the reason the scheme rule is at the boundary. |
| 1.8 | 2026-09-06 | — | **Two clauses amended to state what the platform does** (feature 043). No behaviour changed and no clause was weakened: each amendment was written from the measured bound and then read back against it. **FR-RTM-09** gains the word best-effort and the registry-unavailable case — `connections.ts` admits the connection and logs that five was not checked, because the registry is not a source of truth (principle IV), so the clause now says a user may exceed five while it is unreachable. **FR-RTM-10** gains the degraded-mode bound ADR-20 had already derived and the SRS had never carried: under fabric loss revocation rides a 60-second per-connection re-read (`DEFAULT_REREAD_INTERVAL_MS`, asserted at `membership.test.ts:16`), which exceeds the five-second promise by 55. Both were findings in `docs/09-platform-implementation-review-2026-09-03.md`, which asked for an explicit amendment rather than a silent divergence — and the governance clause requires exactly that. The review's own quota and webhook-remedy rows were corrected in the same pass, both annotated in place rather than rewritten. |
| 1.9 | 2026-09-06 | — | **NFR-SCL-01 discharged by measurement.** The clause — 10,000 concurrent WebSocket connections per gateway instance — has been P1 and unverified since 1.0. Its verification method is `A`, analysis including load testing, and that is what was run: one gateway instance sustained **10,000 real client sockets for 90 seconds** past two 30-second ping intervals with zero closures at 160 MB RSS, and **20,000** also held. No clause changed. Three things are worth recording beyond "met". The subscription cost is exactly `5 × channels + 1 × connected users`, so **which term dominates is a property of the customer's data rather than of the platform** — 9% of subjects at 200 channels, 83% at one channel per user. `DEFAULT_LIMITS.connect` is 3,000 per minute against a gateway that accepts 1,125-1,675 per second, so **filling an instance from cold is bounded by policy at 3m20s where capacity would take 7.7s** — deliberate, and never before stated where an operator planning a rolling restart would find it. And the measurement supplies the threshold the implementation review asked for on subject-grammar consolidation. Full record: `docs/11-scalability-measurement-2026-09-06.md`. |
| 1.10 | 2026-09-06 | — | **Three clauses amended to publish the remedy beside the limit** (feature 044). Chapter 3.23 documented a soft edge — a message older than a client's cursor that changed during a disconnect produces no frame **and no sequence gap** — and recorded history as the repair. Nothing told a client when to perform it, so the limit was published and the remedy was not reachable from it. **EIR-WS-03** now enumerates the ack's third field, a per-channel revision count reported for every channel of which the user is a member, including those at zero: a client with nothing to repair still needs the baseline it will compare against next time. **FR-RTM-03** states the limit in the clause that describes what resume delivers, rather than only in this appendix, and names the count as what a client compares — and states the two negatives that make the signal safe: the platform performs no comparison and refuses no connection over a number a client supplied, so a fabricated count cannot become a denial of service the client controls. **FR-RTM-05** says where an edit or a deletion reaches a client that was not connected, which is the count and not a replayed event. No behaviour was weakened and no clause was narrowed; the platform gained one column, `channels.revision_sequence`, raised by exactly one per revision inside the transaction that applies it, and **not raised by a send** — counting sends would make every active channel report a repair after every absence. The measurement behind SC-004 is `docs/11-scalability-measurement-2026-09-06.md`; the counts ride `/internal/session`, a call the gateway already makes, because at 10,000 connections a per-channel query per handshake is the cost this could not pay. **The task that ordered this amendment named the wrong clauses**: it said SRS FR-016a and FR-016b, which are chapter 3.23's specification ids and appear nowhere in this document — the SRS carried that limit in revision 1.6's narrative and in FR-RTM-03's silence. Reading the clauses rather than the identifiers found three to amend where the task expected two. |
| 1.11 | 2026-09-14 | — | **FR-ANL-07 amended to drop "truncated payload"** (feature 049, chapter 4.4). The clause asked the API request log to record a truncated payload, and two others forbid it: **FR-ANL-11** keeps message content out of the analytical store, and **constitution VI** keeps it out of logs. `POST /v1/channels/{id}/messages` has a body whose whole content is message text, so a request log that truncates it stores message text — and truncation makes it worse rather than better, because it keeps the beginning, which is the part a person wrote. The alternative considered and rejected was an allow-list of endpoints whose bodies are safe: that is a hand-maintained table, it **fails open** because a new route is included by default, and this project has already paid for a hand-maintained table once in the nine api ports. What the clause loses is real and is named rather than glossed: a request log with no payload cannot answer *"what exactly did they send"*, only *"what did they call and what happened"*. The producer built for this clause records identifiers, statuses and durations, and its shaper names every field individually rather than spreading its input — an allow-list fails closed when somebody adds a field, a spread fails open. |
| 1.12 | 2026-09-15 | — | **DR-11 amended to carry the bounds it was being credited with** (feature 050, chapter 4.5). The clause read *"Inserts shall be batched or use asynchronous insert mode; single-row synchronous inserts are prohibited"* — no interval and no row count — while `docs/05-sad.md` §4 described the ingester as batching *"every 2 s or 10k rows (DR-11)"* and `services/ingester/src/ingest.ts` stated those figures as the clause's own. Two hops, and the code comment was the furthest-travelled copy. The numbers are now here, with 049's measurement of where the two bounds cross (5,000 records/second) as the reason it takes both. **And this document's header had read `Version: 1.3` since that revision while the table below ran to 1.11** — `check-revision-order` reads the table and never the header, so eight revisions passed with nothing to notice. Corrected in the same pass. |
| 1.13 | 2026-09-15 | — | **Appendix C open question 4 closed, FR-ANL-05 given its unit, and a constitution conflict recorded** (feature 051, chapter 4.6). The question — per-second precision or per-minute rounding — had been open since before Part 4 and was owned by *Product / Billing*; the chapter that puts connection-minutes in a billing rollup is the one that cannot leave the unit undefined. **Closed as a calendar bucket**, which is what `meter.ts` has billed since 3.24, on a measurement rather than a preference: 56 calendar minutes against 0.6513 elapsed over 55 real connections, **a factor of 86**, because the median connection lasted 252 ms. FR-ANL-05 states the unit now instead of leaving it to an appendix. **AND A SECOND THING IS RECORDED RATHER THAN AMENDED**, because it is the constitution's to change and not this document's: constitution III says *"Analytical queries MUST NEVER execute against the operational database (PostgreSQL); billing, metering, and dashboard analytics read only from the analytical store (ClickHouse), fed via a durable queue (SRS CON-01)."* Measured at chapter 4.6: metering reads **only** Postgres — `usage_periods` is a meter in the operational database, maintained synchronously on the send path — and `message_events`, the source of three of FR-ANL-05's four quantities, has **no producer at all**, its only writer being `scripts/scale/load-analytics.mjs:53`, which is `postgresql('${PG_HOST}', …)`: ClickHouse executing a query against Postgres, the clause's first prohibition rather than its queue qualifier. **So the only way to demonstrate the rollup the clause asks for is to run the thing the clause forbids.** `docs/12` §4 justifies the Postgres counter existing — a quota must refuse a send synchronously, so its counter cannot live downstream of a lossy stream — and justifies nothing about the sentence above it. Constitution VII requires the conflict *"resolved explicitly by amendment rather than silent divergence"*; the amendment is the constitution's and is named here so the next feature inherits a decision rather than a discovery. **AND THIS REVISION REPRODUCED 1.12's OWN DEFECT WHILE WRITING IT.** That entry records the header reading `1.3` while the table ran to `1.11`, because `check-revision-order` reads the table and never the header. This row was added and the header left at 1.12 — and `check:docs` passed. Corrected by looking, which is the only instrument there is for it. **DR-09 and DR-10 are also amended.** DR-09's *"daily aggregates for 25 months"* had been unimplemented since the first draft and filed as a missing clause twice — `gaps.md` 047-3 and 048-4 both say *"no clause says how long metering history is kept"*, and both were quoting DR-10, the next row down. Applied at chapter 4.6 as a 25-month TTL on both rollups. DR-10 now states the two conditions a rollup must meet to satisfy it, because a view that conforms to its letter satisfied it over a table receiving no events. |
| 1.14 | 2026-09-16 | — | **FR-ANL-06 amended where this chapter's measurements show the clause as written cannot hold** (feature 052, chapter 4.7), and the constitution conflict 1.13 recorded is now named in its second, sharper form. The clause asked for agreement *"to within 0.1%"* between metered totals and *"counts derived from operational data"* and left four things to the reader. **(1) WHICH OPERATIONAL COUNT.** `messages` (through `channels`) and `usage_periods.messages_sent` are both derived from operational data and they disagree: **19,012 against 18,962, 0.2630%** across the lane. Every disagreement is attributable — a raw `INSERT INTO messages` in a fixture bypasses the counter, a hard `DELETE FROM messages` removes a row the counter already counted — and **0 of 1,385 tenant-periods disagree for a non-fixture reason**, because `sendMessage` writes the message and increments the counter in one transaction. The clause names its source per quantity now, and the report names the table it read. **(2) PER TENANT, NOT IN AGGREGATE.** 0.2630% is the number a reconciler that sums first would publish; underneath it **49 tenant-periods breach** and one is wrong by 100%. **(3) ABSENCE IS NOT DISAGREEMENT.** `not-comparable` (no operational counterpart exists — the stored message count has none) and `no-data` (this tenant has nothing on either side) are verdicts of their own. Measured here: **every tenant in the platform is in one of the two.** **(4) THE BOUND IS UNREACHABLE FOR THREE OF THE FOUR QUANTITIES.** `uniq` is exact to **65,536** distinct and 0.5676% at 65,537 — the cliff is at 2^16 and one distinct user wide, which sharpens `gaps.md` 047-1's *"exact to 60,000"*. The retention boundary makes the oldest day in any window disagree by **0% at midnight and 1.0989% just before it**, on the same data, which sharpens 047-1's single 0.49% into the curve that figure sits on. And connection-minutes cannot agree while any deploy leaves established sockets to the process exit, which chapter 4.5 decided is correct. **These are clauses that cannot be met rather than defects to fix, and the chapter publishes no percentage that implies otherwise.** **AND CONSTITUTION III, A SECOND TIME, AND THE ARCHITECTURE HAS ALREADY ASSUMED THE ANSWER.** The clause says *"billing, metering, and dashboard analytics read only from the analytical store"* while FR-ANL-06 requires comparing against PostgreSQL. The reading that makes both true is that **the reconciler is none of those three roles — it is an auditor, and an auditor confined to one side of a fence cannot check the fence.** That reading is not this document's to ratify; it is the constitution's, under VII's *"resolved explicitly by amendment rather than silent divergence"*, and it is named here so the next feature inherits a decision rather than a discovery. What is new since 1.13 is that **`docs/05-sad.md`'s ADR-06 has said it all along**: its accepted trade-off reads *"mitigated because the only strict consumer (metering) reconciles daily against Postgres (FR-ANL-06)"* — so the cross-path read is the mitigation that makes choosing NATS over Kafka acceptable, not an oversight, and removing it would cost that ADR its argument. **AND A CITATION IN THIS DOCUMENT POINTED AT NOTHING.** DR-09's 1.13 amendment cited *"FR-003a"* as though it were a published clause. **There is no FR-003 in this document**; `FR-003a` is a feature-local id used by four different features to mean four different things. Corrected to name the decision instead of the id, here and at `docs/05-sad.md:879`, where the same citation sat beside a sentence 4.6 had already falsified — *"the rollups carry no TTL"*, when DR-09's 25-month TTL was applied to both in that chapter. |
| 1.16 | 2026-09-17 | — | **FR-ANL-06's verification status recorded, and it covers one of the clause's three parts** (feature 054, chapter 4.9). The clause asks for a comparison *within 0.1%*, *verified by a daily job*, *that alerts on breach*. **The platform has the first.** The planted-drift suite runs on every push and the bound was measured at a volume where 0.1% is a threshold rather than a rounding of nothing: **121,057 against 121,057, 0.0000%**, where the smallest expressible drift is **122 messages** — planted at 121 it passes and at 122 it breaches, in both directions, through the real job. **The daily job has no runner of any kind**, which no document had recorded: zero occurrences of `reconcile-usage` in either repository's `package.json`, in `turbo.json`, in `ci.yml` or in any `*.sh`, and `ci.yml` triggers on `push` and `pull_request` only. ADR-28 decides that, and decides it as a recorded absence — a daily sweep today would report `no-data` for every tenant, because no environment in the platform has both sides of the comparison. **AND THE GATE THAT CARRIES THE `T` COULD NOT SIGNAL.** `pnpm test:integration` had been red on every run since chapter 4.4 for six failures unrelated to metering, so a planted drift made a red run redder — **the signal was indistinguishable rather than absent**, which is not what four chapters recorded — and `--concurrency=1` meant five lanes had not run at all. ADR-27 fixes both and makes the gate report **suites executed against suites present** (54 of 54), because turbo's own summary counts tasks and does not reproduce run to run. **One unset environment variable was the whole of two of the six failures**: `limits.itest.ts` said so loudly, while three attacks in the cross-tenant suite constitution VI names as gating releases returned at their first line and reported green — 1ms, 0ms and 1ms against 27ms, 6ms and 33ms once it was set. **And constitution III's fourth bullet is still unamended**, now with a written proposal (`specs/054-chapter-4-9/constitution-amendment.md`) rather than a third deferral: it says 0.1% flat and a daily job that alerts, where this document has said since 1.14 that three of four quantities cannot meet the bound and that the alert has no mechanism. |
| 1.15 | 2026-09-17 | — | **Three analytics clauses amended where this chapter's measurements falsify them, and one architecture decision recorded** (feature 053, chapter 4.8). **FR-ANL-10 had no quantity.** It asked for *"end-to-end delivery latency percentiles"* and the phrase has three readings; the platform's one existing latency measures a different one, which `analytics/0003_webhook_attempts.sql:20` has said since it was written — `deliver.ts` times the `fetch` alone, so `webhook_attempts.latency_ms` excludes the commit, the outbox, JetStream, the claim and every retry gap. The clause now names **commit to the frame written to a subscriber's socket**, which is the real-time path constitution II is about and the reading `message_events.delivery_latency_ms` was created for; the two readings not chosen are recorded with what each would cost. **The producer still does not exist and the clause says so.** That column holds 0 rows and has 0 producers (`gaps.md` 048-2, carried through six features and re-measured here — 4.7's *"0 occurrences under `services/`"* is now 3, and all three belong to one test, so *"0 producers"* is what was always meant). The one thing that writes the table, `scripts/scale/load-analytics.mjs`, supplies `CAST(NULL AS Nullable(UInt32))` for that column in both halves of its `UNION ALL`. Building it needs both instants carried across a service boundary on the busiest path in the platform. **AND THE CLAUSE NOW FORBIDS `quantile`.** Measured against the server: over a uniform sample the p50 error is **16.67% at n=4** and is **not monotone** — 0 at n=155 and back to 0.90% at n=10,000 — and over a skewed sample, which is what a latency is, it **grows** with the sample to **9.52% on all three percentiles at n=10,000**. On the platform's one real latency sample, `webhook_attempts` at n=64, `quantile(0.99)` answers **4,961.26** where `quantileExact(0.99)` answers **10,000** — **50.39% low**, and low is the direction in which an alert threshold never fires. **This is the opposite shape from `uniq`**, which is exact below 65,536 and wrong above: there is no n at which `quantile` is safe. And the clause's own grain is the small one — per tenant per hour over the request log gives **178 buckets, median 10 rows, 73 of them under five** — so any implementation publishes `n` beside every percentile. **FR-DSH-03's twin had been amended without it.** Revision 1.11 struck *"truncated payload"* from FR-ANL-07 because FR-ANL-11 keeps message content out of the analytical store and constitution VI keeps it out of logs; FR-DSH-03 went on asking the dashboard for a request log *"with full request and response detail"*, and **EIR-DSH-02 permits the dashboard no other source than the public API**. The three could not all hold. *"With full request and response detail"* is struck, carrying 1.11's own sentence rather than writing a new argument: *a request log with no payload cannot answer "what exactly did they send", only "what did they call and what happened"*. **An amendment that fixes one clause and leaves its twin standing is the defect, not the clause** — and this is the third time this project has found that shape, after DR-09's second half filed as absent twice and `FR-003a` cited as a clause in two documents. **FR-ANL-08's ninety days cannot exist over `api_requests`.** FR-ANL-07 retains that table for 30 days. Measured: two rows planted at `now - 60 days` and `now - 1 day` leave **one survivor**, and `OPTIMIZE TABLE … FINAL` changes nothing, because the TTL removes rows **at INSERT** (chapter 4.2's finding, reproduced). So no fixture can put a 90-day window in front of the clause **at any volume**, which is stronger than the lane being small. Where the two are read over the same table, retention governs and the window is stated beside the figure. **AND ADR-26 IS NEW, IN BOTH DOCUMENTS.** The api now serves a customer request from the analytical store — the first time the platform's availability depends on ClickHouse answering, and a failure mode it did not have. Its rejected alternatives are measurements: a client-side deadline alone leaves the query running (`SETTINGS max_execution_time = 1` answers **HTTP 408 in 1.002 s**), `LIMIT BY` changes what `LIMIT` counts, and Postgres has nothing to serve. **`FINAL` costs 3 ms against 2 ms at seven parts**, measured with merges stopped rather than on a table that had just been merged. Its reversal condition is the analytical store's availability appearing in the API's error budget, and the 503 `analytics_unavailable` refusal shipped in the same chapter so the signal exists before the decision needs reviewing. **AND A CITATION IN THIS FEATURE'S OWN DRAFT REPRODUCED 1.14's DEFECT.** Two published paragraphs were written citing `FR-025` — a feature-local id, not a clause in this document — three months after 1.14 corrected `FR-003a` for the same reason. Caught by running the check rather than by reading: every prefixed identifier this feature cites resolves, and the bare-numbered ones do not, which is the only difference between the two classes. Corrected to name the decision instead of the id. |
