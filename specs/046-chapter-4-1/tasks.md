...
- [X] T010 **DONE** — `f0f721f` docs(046): chapter 4.1 specified, planned, and phase 1 measured. Original: Commit phase 1 in `relay` — `specs/046-chapter-4-1/baseline.txt` only. No platform change yet.
...
- [X] T019 **DONE** — `28020be8` feat(046): the corpus seeder, and six things only running it could say; recorded in `339eba2`. Original: Commit phase 2 — `relay-platform/scripts/scale/corpus.mjs`. Gates first: `pnpm lint && pnpm typecheck && pnpm test` in `relay-platform`.
...
- [X] T032 [US1] **DONE** — `ff20caeb` feat(046): the measurement, and the hypothesis it falsified twice; recorded in `7e9f213`. Original: Commit phase 3 — `relay-platform/scripts/scale/measure.mjs` and `specs/046-chapter-4-1/baseline.txt`. Gates first.
...
- [X] T042 [US2] **DONE** — `856af98b` feat(046): the fix's storage cost, after three numbers that conflated two things; recorded in `2032867`. Original: Commit phase 4 — `relay-platform/scripts/scale/measure.mjs` and `specs/046-chapter-4-1/baseline.txt`. Gates first.
...
- [X] T050 [US3] **DONE** — `04fe516a` feat(046): the seeder names its failures, and refuses a name it cannot quote; recorded in `f67b305`. Original: Commit phase 5 — `relay-platform/scripts/scale/corpus.mjs` and `specs/046-chapter-4-1/contracts/seeder.md`. Gates first.

---

## Phase 6: The chapter, and closing out

- [X] T051 **DONE.** `the-question-the-counters-cant-answer`, both locales. Original: Choose the chapter slug and create `relay-tutorial/app/(en)/part-4/chapter-01/<slug>/page.mdx`. The directory convention is the global ordinal decided in `docs/12-part-4-structure.md` §2.1.
- [X] T052 **DONE.** Opens on Part 3's counters and asks which FR-ANL question they answer. The answer is one. Original: Write the opening of `relay-tutorial/app/(en)/part-4/chapter-01/<slug>/page.mdx`: the questions FR-ANL asks, and which of them Part 3's counters can answer. The answer is one, and the reader built those counters.
- [X] T053 **DONE.** The shape mismatch, with chapter 2.4's dropped-index comment quoted from `schema.ts` — **named by subject, not ordinal** (045 FR-008). Original: Write the shape-mismatch section of `.../page.mdx`, including the callback to the chapter on what a user sees — an index measured and dropped on camera in migration `0001`, with the schema's own comment quoted. **Name the subject, not the ordinal** (045 FR-008): the rework renumbered twenty-one of twenty-six chapters.
- [X] T054 **DONE.** M1–M4 with plans, corpora and conditions; the lane figure beside the corpus figure and **the factor stated as 651** (SC-002). Original: Write the measurement sections of `.../page.mdx` carrying M1–M4 with their plans, corpora and conditions, and the lane figure beside the corpus figure (FR-008, FR-009). **State the factor between the lane figure and the corpus figure as a number** (SC-002) — the predecessor's pair was 0.87 ms against 159 ms, and the factor is what makes the point rather than the two durations.
- [X] T055 **DONE — AND THE SECTION ARGUES THE OPPOSITE OF THE PLAN.** The index removes the join and leaves the sort, so it buys a difference inside the run-to-run spread for +49% storage. Original: Write the counterfactual section of `.../page.mdx`: the index helps, and here is the bill. **This is the chapter's argument and the reason it is not answerable with "add an index."**
- [X] T056 **DONE.** `<ForwardRef>` names what the chapter did not do and cites the fire-and-forget path rather than re-deriving it (FR-011, FR-012). Original: Write the forward reference in `.../page.mdx` naming what this chapter did not do — no ClickHouse, no emission path, no reconciliation (FR-012). And do **not** re-derive the fire-and-forget tradeoff: the chapter on when to stop trying shipped `analytics.{domain}.{action}.{environment_id}` and argued it (FR-011).
- [X] T057 [P] **DONE.** Three figures — the two shapes, where the time is, what the fix costs. **`check:figures` caught all three passing `src` where `Figure` reads `code`**; they would have rendered empty. Original: Write `relay-tutorial/app/(en)/part-4/chapter-01/<slug>/figures.ts` — at least two figures: the two index shapes against the two questions, and the four measurements as a before/after.
- [X] T058 [P] **DONE.** Three `TRAP` boxes: the two predicates between the volume asked for and the volume measured, the two wrong neighbour measurements, and the three wrong storage numbers. Original: Add at least one `TRAP` box to `.../page.mdx`. The candidate with the best evidence is the lane's own answer: 0.87 ms would have settled the predecessor's question wrongly, and it is the same trap here.
- [X] T059 **DONE.** Part 4 renamed to **Everywhere the data went** with a Vietnamese title, and 4.1 registered. `translatedIn` deliberately omits `vi` — a placeholder is not a translation. Original: Register the chapter in `relay-tutorial/lib/tutorial.ts` — path, title, and both locales — and add Part 4's title, "Everywhere the data went", with a Vietnamese rendering.
- [X] T060 **DONE.** Placeholder via `vi-placeholder.py`, **18 fence delimiters in both locales**. 045's copy died on `id="(3\.\d+)"`; copied here and generalised — see `gaps.md` 046-5. No Vietnamese was invented beyond the standing notice and the registry's own title. Original: Create `relay-tutorial/app/(vi)/vi/part-4/chapter-01/<slug>/` carrying every code fence byte-identical to the English page (045 FR-010). If it ships untranslated it must be a visibly marked placeholder (045 FR-011).
- [X] T061 **DONE. 2,132 prose words**, inside the 2,000–4,000 bound (SC-008). Original: Run `node scripts/prose-words.mjs` in `relay-tutorial` against `.../page.mdx` and record the count in `specs/046-chapter-4-1/baseline.txt`. The bound is 2,000–4,000 words measured **outside** code fences (SC-008).
- [X] T062 **DONE. 110 problems, APPLY 74, HEAD 36 — a delta of 0** against T006's opening. The chapter fences no platform file, so it adds nothing to the chain. The total is not claimed green (FR-015). Original: Run `pnpm check:fences` and record the closing number in `specs/046-chapter-4-1/baseline.txt`. **Report the delta against T006's opening, not the total** (FR-015). The total is not claimed green and the chapter's close-out says so in words.
- [X] T063 [P] **DONE.** `lint`, `typecheck`, `test`, `build` in `relay-platform`; `check:docs`, `check:srs`, `check:figures`, `check:fences`, `check:errors` in `relay-tutorial`. All pass. `check:errors` was run **after** the platform build, since it reads `packages/protocol/dist`. Original: Run the remaining gates and record each result in `specs/046-chapter-4-1/baseline.txt`: `lint`, `typecheck`, `test`, **`build`** in `relay-platform` **first**, then `check:docs`, `check:srs`, `check:figures`, `check:errors` in `relay-tutorial`. **The order is load-bearing** — `check-error-codes.mjs:30` reads `relay-platform/packages/protocol/dist/codes.js`, so `check:errors` run before a build reports on the previous build's registry.
- [X] T064 **DONE.** `traceability.md` — 19 requirements, 8 criteria, with the verification method actually used. Original: Write `specs/046-chapter-4-1/traceability.md` mapping FR-001…FR-015 and SC-001…SC-008 to the tasks that verify them.
- [X] T065 **DONE.** `gaps.md` — seven entries, including R4's reconciliation divergence, the two unbuilt gates, and the six unwired instruments. Original: Open `specs/046-chapter-4-1/gaps.md` and record what this chapter found and did not close: research R4's reconciliation divergence, which movement IV inherits; the two gates `docs/12-part-4-structure.md` §6 says must exist before Part 4's first chapter split; and **the reuse claim SC-006 used to make** — that a later chapter runs this seeder unmodified — which only movement IV can settle.
- [X] T066 **DROPPED BY DECISION, 2026-09-13 — not deferred.** The reader protocol asked for an hour of a person who has not read the plan. Fifteen records named it and none ran it. **The decision rests on a cost that was checked**: 4.1 contributes 0 titled fences, so a prose correction after publication moves no platform commit, invalidates no tag and re-runs no chain — feedback on the writing arrives from readers and is applied then. What is given up is stated in `gaps.md` 046-2 rather than elided. Original:  **NOT RUN, AND IT IS THE ONLY TASK IN THIS FEATURE THAT NO COMMAND CAN CLOSE.** It needs one person who has not read this specification, given the published chapter and nothing else, answering SC-007's three questions in their own words — `specs/036-chapter-3-18/reader-protocol.md`, 45 minutes. **Fourteen records named this gap before this one and none closed it**; `gaps.md` 046-2 is the fifteenth. It matters more here than usual: **this chapter's argument changed twice during measurement** — the neighbour effect falsified, then the index's benefit falsified — and the prose was rewritten each time by the person who took the measurements. Every check in these repositories compares bytes. Original:  **Run the reader protocol** per `specs/036-chapter-3-18/reader-protocol.md`: one person who has not read this specification, given the published chapter alone, answering SC-007's three questions. Record every question they could not answer verbatim in `specs/046-chapter-4-1/gaps.md`. **Fourteen records have named this and none has run it.**
- [X] T067 **DONE, AND THE BLOCK IS LIFTED.** Part 4 uses `part4-chN` — the ordinary convention, no `rework/` prefix. `part4-ch1` is cut as an annotated tag on `04fe516a`. **Part 3's twenty-one stale `part3-chN` tags remain**, so the namespace is right for Parts 1, 2 and 4 and wrong for Part 3; recorded as `gaps.md` 046-3 with its three possible answers. Original: **DONE.** Recorded as `gaps.md` 046-3. Original: Record in `specs/046-chapter-4-1/gaps.md` that the chapter **cannot be tagged**: `part3-ch18` is `54b2cd53` and `rework/part3-ch18` is `3732d6cf`, both exist, and `README.md:8` promises one tag per chapter. Name it as blocking Part 4's tag convention rather than this chapter's content.
- [X] T068 **DONE.** Final sweep: 0 corpus databases, the lane identical to T007 in all four columns, migrations ending at `0014_connection_minutes.sql`. Committed across all three repositories; `CLAUDE.md`'s `<!-- SPECKIT -->` block carries the close-out figures, and its **USE A PERSON** section is amended to record a decision rather than repeat an intention. Original:  Confirm one last time that no `relay_corpus*` database remains and the lane's row counts match T007's, then commit phase 6 in all three repositories and update the `<!-- SPECKIT -->` block in `CLAUDE.md` with the chapter's close-out figures. **The final sweep can fail for a real reason** — a phase whose cleanup task did not run — which is the only kind of check worth adding after two that already passed.
