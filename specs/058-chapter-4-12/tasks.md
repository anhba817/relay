...
- [ ] T005 **Re-measure the reference lookup before the index exists**, and keep the numbers: `EXPLAIN (ANALYZE, BUFFERS)` for a hit and for a **miss**, in the JOINED and SCOPED form T010a ships rather than the bare one `research.md` measured. **And parameterise it**: the planning figures came from a literal `@>` and the driver sends a bound parameter. Checked at analysis pass 2 — `PREPARE p(jsonb) … EXECUTE` gives the same `Bitmap Index Scan` at 6 buffers, so the figure survives the form the code uses. That is the premise most likely to have silently invalidated the chapter's headline number, and it held. The planning figures were 2.132 ms / 806 buffers and 2.886 ms / 1,016 on 66,516 messages. **The lane moves between chapters** — 4.8 watched a median shift because the feature itself changed the population — so a figure carried from `research.md` into the chapter without re-measurement is a figure about a different database.
