...
- [ ] T017 [US1] Sign a **GET** with `expiresIn: 3600` in `services/api/src/media/media.service.ts`, using the `object_key` T010b's query returns. `presign.ts` has taken `"GET"` since 4.10 and needs no change — assert that it needs none rather than editing it to be sure.
