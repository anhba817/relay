...
- [X] T004b **DONE, AND IT FOUND A RED NOTHING ELSE COULD.** 125 files, 1,778 tests, 664 s. `packages/config/src/infra.test.ts` was red because this chapter added a container and did not register it in `@relay/config` — **the assertion that caught it was written by the chapter that added the fifth container.** No suite this feature ran touches that package. The other five failures are the lane's own: dispatcher 16 of 16 and typing 23 of 23 alone. Original: End this phase by running **`pnpm coverage`**, not only `pnpm test:integration`. The twin is only visible from the run that uses it, and eight minutes is what finding out cost last time.
