...
- [ ] T024 [US2] **DEFERRED TO PHASE 5, AS THE TASK ITSELF SAYS.** The condition is unreachable until `storage_bytes` is configurable, and a test of it cannot fail. Original: Refuse when committed bytes plus the declared size exceeds the environment's cap. **Depends on phase 5's configuration**; until then the condition is unreachable and a test of it cannot fail.
