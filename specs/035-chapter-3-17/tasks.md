...
- [ ] T017 [US1] Add `kind` and `description` to `upsertUserEntrySchema` in `services/api/src/users/users.schema.ts` — `kind` optional defaulting to `person`, `description` required when `kind` is `bot` and refused when it is `person`
...
- [ ] T019 [US1] Refuse a kind change in `services/api/src/users/users.service.ts` with `field: "users.N.kind"`, naming the failing entry's index the way the metadata bound already does
...
- [ ] T024 [P] [US1] Test that a kind change is refused in **both** directions, and that the per-entry result names the failing index in a batch — `services/api/src/users/users.itest.ts`
...
- [ ] T040 [US3] Refuse the mint for a bot in `services/api/src/auth/dev-token.controller.ts`, **404 and with the body an unknown identifier gets**. A refusal that says "this is a bot" is an oracle for which identifiers are bots — the same argument chapter 3.16 made for not distinguishing created from existed
