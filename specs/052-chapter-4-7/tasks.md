...
- [ ] T003 [P] Re-run R4's tenant counts **and attribute every analytical environment to the application that owns it**. Planning measured 4 against 675 and then found all four to be test fixtures absent from Postgres, so the real figure is **0 against 675**. **Join to `applications` before believing any count** — three numbers in this feature turned out to be about the lane rather than the platform, and this query settled all three.
