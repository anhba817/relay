...
- [ ] T035a [US2] **Load a corpus, because the store cannot answer SC-002 as it stands.** Measured at T001: `message_events` **0 rows**, `connection_events` **154**. A rows-read figure against an empty table is not a measurement, and 4.2's cited 315-against-1,052,655 exists because that chapter loaded one. Run `scripts/scale/corpus.mjs` and then `scripts/scale/load-analytics.mjs`. **Record the corpus database name, the row counts it produced, and the environment ids it created** — 047's pass 9 found every number of that feature correct about a database the chapter had not loaded.
- [ ] T035b [US2] **Record the bind this creates, because it is the chapter's sharpest argument and not an inconvenience.** `load-analytics.mjs:53` is `postgresql('${PG_HOST}', …)`, which is the cross-path read T053 names as constitution III's first prohibition. **So the only way to demonstrate that the rollup works is to execute the violation the chapter is about.** The clause is not merely unmet — it is unmeetable, and the demonstration needs the forbidden path. That is a `<Trap>` candidate (T066) and it belongs in T053's amendment.
...
- [ ] T036b [US2] **Remove the corpus rows and verify both directions**, before anything else is counted. `load-analytics.mjs` writes into the shared `relay_analytics` — `DB = "relay_analytics"`, not a scratch database — and every later suite reads those tables. **050-2 is exactly this missing guard**: Postgres has a trigger, an exemption list asserted both ways and a sweeper, and ClickHouse has none of the three. Delete by the environment ids T035a recorded, then assert the counts are back to their T001 openings **and** that the rollup holds no rows for those ids. Research R8's planted row was removed from the source *and* the view's inner table, because a materialised view does not propagate a delete.
...
- [ ] T041 [US3] Discharge **FR-001b**: write `relay-platform/analytics/0008_mv_messages.sql`, the second view, over `message_events`, writing `messages`, `active_users_state` and `stored_delta` into the same target. **Even though its source has no producer** — the view is what makes the rollup complete the day a producer exists, and R1 measured two views into one target summing correctly.
