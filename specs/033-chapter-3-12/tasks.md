...
- [ ] T051a [US5] Add `channel_member_limit_exceeded` to `ERROR_CODES` and enforce FR-CHN-07 in `services/api/src/channels/channels.service.ts`: a channel holds at most 1,000 members, and an add that would exceed it is refused with `422` and that code. The SRS states the number, the status **and** the requirement of a specific code, and its own worked example for EIR-API-04 names the code — so all three were written down and no artifact mentioned any of them until analysis pass five. The shipped code set becomes **thirteen** (FR-048, SC-033)
- [ ] T051b [P] [US5] Test the boundary from storage: adding the 1,001st member is refused and the channel still holds 1,000 afterwards, read back rather than inferred from the status (FR-048, SC-033)
