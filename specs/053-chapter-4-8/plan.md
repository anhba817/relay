...
| **VII — Governance** | **PASS on the amendment bullet, and a second bullet needed work** | FR-ANL-10 is amended rather than approximated, following revisions 1.12–1.14, and constitution III's own amendment stays 052-6's. **The principle also says every architecture decision is recorded as an ADR with drivers, rejected alternatives and a reversal condition** — and putting the analytical store on a customer request path is one: a new runtime dependency and a new failure mode where there were none. Chapter 4.7's precedent covers a batch job, not this. FR-031 and T056a–T056d close it; the reversal condition is the part nobody had written. **Third principle in this feature whose heading passed while a bullet went unread**, after I's cross-tenant suite and III's availability sentence. |
