...
- [ ] T001a **Open `.github/workflows/ci.yml`, which nine analysis passes did not** (FR-033, SC-021). Record what the `platform` job provides: `postgres:18-alpine`, `redis:8-alpine`, `nats:2.12-alpine` — and **`clickhouse` appears zero times in the whole workflow**, no service, no env var, no job. Four suites reach the analytical store and none can pass there: `ingest.itest.ts`, `metering.itest.ts`, `request-log.itest.ts` (4.4) and `reconcile.itest.ts` (4.7). **This feature plans a fifth.**
...
- [ ] T074a In `gaps.md`, record the tension the **next** chapter inherits: constitution V says *"every metered unit is visible in the dashboard the moment it is counted"* and III says billing, metering and dashboard analytics *"read only from the analytical store … fed via a durable queue"*. **Immediacy and a queue cannot both hold.** It is not this route's problem — a request log is not a metered unit — but this chapter is the first customer-facing read of that store, and the usage dashboard is the next one. File it forward so that chapter inherits a question rather than discovering one, the way 047-1 was filed for movement IV.
