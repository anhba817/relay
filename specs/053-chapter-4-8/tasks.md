...
- [ ] T035a [US1] **Plant the same request twice and assert the page returns it once** (FR-007's real failure mode). Insert one row with an identical `(environment_id, ts, request_id)` into a second part, assert the surface returns one, and **run it red against a statement without `FINAL`** — the duplicate this lane already held was removed by a merge, so the test has to make its own.
