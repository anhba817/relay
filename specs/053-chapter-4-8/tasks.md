...
- [ ] T013 Write `contracts/request-log.md`'s open decisions down as decisions: the `Accepts` scope and the `/internal/*` treatment. Both carry arguments already; this task turns them into sentences the code must satisfy.
- [ ] T014 [P] Add the query schema in `services/api/src/request-log/request-log.schema.ts` — `from`, `to`, `cursor`, `direction`, `limit` — reusing `historyQuerySchema`'s shape and bounds (1–200, default 50) rather than inventing a second pagination vocabulary (FR-019).
...
- [ ] T025 [US1] Select exactly the six fields FR-ANL-07 names, and decide in the code — with a comment — whether `principal_kind`, `refused_at` and `limited_operation` are returned. The table carries four columns the clause does not name.
...
- [ ] T028 [US1] Add `services/api/src/request-log/request-log.controller.ts` with `@UseGuards(CredentialGuard)` and T013's `Accepts` decision, wired into the module graph.
...
- [ ] T030 [US1] Write the integration suite `services/api/src/request-log/request-log.itest.ts` with a `beforeAll` positive control — ask the store `SELECT 1` before trusting anything else it says.
- [ ] T031 [US1] Plant rows for this suite's own environment ids and **verify the cleanup by count**, not by issuing the delete. `ALTER TABLE … DELETE` is a queued mutation, and 4.7 found a row from an earlier run still in the table because the fire-and-forget form leaves no evidence it ran.
- [ ] T032 [US1] Assert the six fields come back for a planted tenant, newest first (SC-001).
...
- [ ] T038 [US2] Assert the `limit` bound is enforced against the real route, both at the maximum and above it.
...
- [ ] T042 [US1] Record that FR-ANL-08 cannot fail here for its own reason: the largest tenant holds 208 rows (R7). Either plant a tenant at a volume where the clause means something, or record the gap as `docs/12` §2.3 does for the 0.1% figure. **Do not publish a p95 from a 208-row tenant and call the clause discharged.**
...
- [ ] T072 Discharge **FR-022** and **SC-011**: report the fence close as a delta against T003's opening, by kind and locale, with the two HEAD classes split. **Note any file whose edit is invisible because the checker reports the first failure per file** — 4.7's `vitest.coverage.config.mts` gained four changes for a delta of zero.
