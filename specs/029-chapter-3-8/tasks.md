...
- [ ] T034b [US1] Record in the chapter that the connect limit conflicts with the constitution's *"deploys cause … at most one client reconnection cycle"* gate, that the gate has **no implementation** — close code 4009 is declared and emitted by nothing — and that FR-045 leaves the drain-grace exemption to whichever chapter builds a drain. **Four codes were declared in chapter 1.3 and never wired**: `rate_limited`, 4008, `request_id` and 4009. This chapter enforces one, closes one, and can now say exactly what the other two are waiting for
