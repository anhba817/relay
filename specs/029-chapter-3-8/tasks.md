...
- [ ] T025d [US2] Raise the threshold in `services/api/src/auth/credentials.itest.ts` and any other suite T004a's measurement shows needs it, and **lower** it in `services/api/src/limits/limits.itest.ts`, which is the only way to test a threshold. Any suite that needs headroom asks for it explicitly and visibly, rather than the default being chosen to suit the suite
