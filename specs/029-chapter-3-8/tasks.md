...
- [ ] T061 Generate every fence from the real files rather than typing them, and confirm `pnpm check:fences` replays the chain onto `relay-platform` (FR-031). Most of what it shows amends a file some earlier chapter fenced; **eight are worth naming because their chains end outside Part 3**, so a reader tracing one goes a long way back: `packages/protocol/src/frames.ts` (1.3), `packages/service-kit/src/index.ts` (1.4), `services/api/src/protocol-error.filter.ts` (last 3.2), `packages/protocol/src/internal.ts` (last 3.6), `services/gateway/src/registry.ts` (3.7), `services/gateway/src/session.ts` (3.7), and — added by later analysis passes — `packages/config/src/infra.ts` and `packages/config/src/infra.test.ts` (both 1.2), `relay-platform/turbo.json` (last 3.5) and `packages/e2e/src/harness.ts` (last 3.5)
...
- [ ] T061b **The exception: a file `fences/post-series.md` already amends.** For such a file the chapter chain stops *short* of HEAD by that amendment, so a `part3-ch7` base carries a pre-image the chain will reject — and worse, a new chapter fence would land under the existing post-series diff and break its pre-image in turn. Five files are amended today: `package.json` (the **root** one, turbo scripts), `services/api/src/auth/credentials.itest.ts`, `consumer.itest.ts`, `signup.itest.ts` and `deliveries.itest.ts`. **Check the list before generating, not after** — `services/api/package.json` is *not* on it, which is worth confirming rather than assuming from the bare name
