...
- [ ] T017a [US1] Extend that test so the two environments carry **different configured limits**, each enforced at its own number (FR-006, SC-003, research R25). The journey map asks for "separate keys and **separate quotas**", and independent counters satisfy only half of that — a shared policy would mean load-testing a dev environment moved production's ceiling. T018c proves an override applies; this proves it applies to **one** environment
...
- [ ] T022a [US1] Assert the error body's five fields are **top-level, not nested under an `error` key** (EIR-API-04 as amended in SRS 1.3, research R27). The document said nested until this chapter checked; the platform has always been flat, and the assertion is what stops the next reader of EIR-API-04 from "fixing" it
