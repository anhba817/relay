...
- [ ] T061 Generate every fence from the real files rather than typing them, and confirm `pnpm check:fences` replays the chain onto `relay-platform` (FR-031). Most of what it shows amends a file some earlier chapter fenced; **six are worth naming because their chains end outside Part 3**, so a reader tracing one goes a long way back: `packages/protocol/src/frames.ts` (1.3), `packages/service-kit/src/index.ts` (1.4), `services/api/src/protocol-error.filter.ts` (last 3.2), `packages/protocol/src/internal.ts` (last 3.6), `services/gateway/src/registry.ts` (3.7) and `services/gateway/src/session.ts` (3.7)
