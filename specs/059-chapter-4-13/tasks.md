...
- [ ] T022 [US1] **Verify the SIZE from the sweep's own `HEAD`** in `services/media-worker/src/verify.ts` (FR-MED-03, half of it). `Content-Length` is the store's own count — measured at analysis pass 1 — so this half needs no bytes and an over-size object is refused before any GET.
