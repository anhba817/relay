...
- [ ] T014 Idempotence by state in `services/api/src/internal/`, **and the refusal is the part worth testing.** A second `ready` for a `ready` object answers 200 and changes nothing; a `ready` for a **`rejected`** object is refused, because the bytes are gone and a 200 would let a stale worker resurrect a state whose object no longer exists.
...
- [ ] T037 [US3] Add ClamAV to `compose.yaml` — **151 MB compressed, 7 layers** (`research.md` R5) — with `INFRA_SERVICES` updated in the same commit (T018's rule). **IN THE DEFAULT PROFILE, WHICH IS THE OPPOSITE ANSWER FROM THE WORKER'S** (T018). The scanner mutates nothing, and the worker's own suites spawn the worker as a child the way `request-log.itest.ts` spawns the ingester — so the scanner has to be up when a bare `docker compose up -d --wait` is all that ran. The worker is profiled because it writes; the scanner is not because it does not.
- [ ] T038 [US3] **The health check must distinguish *running* from *has definitions*, and the two questions have two commands.** Asked of a running clamd at analysis pass 5: `zPING\0` answers `PONG` and `zVERSION\0` answers `ClamAV 1.5.4/28129/Sun Sep 20 06:26:26 2026` — engine, **signature database version**, and **build date**. A check that sends only the first is *"a check that cannot fail for the reason you care about"* — 4.2's `/ping`, 4.9's unset credential and 4.10's bucket are the three precedents. The check refuses a database older than a stated bound.
- [ ] T038a [US3] **Run T038's check red by forcing the bound, not by breaking the scanner.** Manufacturing a definitionless clamd is awkward; the bound is one constant. The red run asserts the check refuses when the reported date is too old, which tests the check rather than ClamAV — and a check nobody has run red is the thing T038 exists to avoid becoming.
- [ ] T039 [US3] Speak `INSTREAM` over the socket in `services/media-worker/`, chunked. No client library: the protocol is `zINSTREAM\0`, then a 4-byte big-endian length before each chunk, then a zero length — proven against a running ClamAV at analysis pass 1. The object may be 100 MB.
- [ ] T039a [US3] **THE SCAN RUNS BEFORE THE DECLARATION CHECK, AND A TEST THAT COULD NOT BE WRITTEN IS WHY** (`research.md` R5a, `data-model.md` §4). Reorder `services/media-worker/src/verify.ts` so the scan precedes T022a's type comparison. Measured through `INSTREAM`: EICAR alone is `Eicar-Test-Signature FOUND`, EICAR plus a newline is `Eicar-Signature FOUND`, and **EICAR plus two hundred spaces, EICAR inside a valid PNG at either end, and EICAR followed by a kilobyte are all `OK`** — the signature matches the file, not a substring. `ALLOWED_TYPES` has no text type, so **no object can both satisfy FR-MED-03 and trip the scanner**. FR-MED-04's own *"every uploaded object"* had already required this order; the impossible test is what surfaced it.
- [ ] T039c [US3] **AND THE FULL ORDER IS SCAN, SIZE, TYPE — STATED HERE BECAUSE FIVE PASSES FIXED IT PAIRWISE.** T022 moved the size comparison onto the sweep's `HEAD` and T039a moved the scan ahead of the *type* check, which together left the size verdict knowable before the scan with nothing saying whether it short-circuits. **It does not.** *"Every uploaded object shall be virus-scanned"* is the clause that ordered the type check, and an object declaring one byte while holding five megabytes is at least as worth scanning as one whose type is wrong — reading the word one way and not the other would be reading it selectively. **The cost is bounded by something that already exists**: streaming up to `KIND_CAPS`'s 100 MB for an object that will be refused, where a caller who wants 100 MB streamed can upload a valid 100 MB video. Assert the order with a test that plants an object wrong on BOTH size and signature and reads `rejected_reason`.
- [ ] T039b [US3] **Write the rejection precedence down**: an object can now fail both, and `scan_failed` wins. It is the more serious fact about the caller and the one an operator reading `rejected_reason` needs. A test plants an object that is both infected and mis-declared.
- [ ] T040 [US3] Integration test in `services/media-worker/src/scan.itest.ts`: **the EICAR signature is rejected** — `rejected`, reason `scan_failed`, bytes gone. The 68-byte string **unpadded and unembedded**, uploaded against a slot declared for any allowed type: the declaration is wrong and that is the point, because T039a puts the scan first. A test that mocks the scanner asserts that the mock was called.
- [ ] T040a [US3] **Assert the scanner's own answer, not just the platform's.** The suite sends EICAR through `INSTREAM` directly once and asserts `Eicar-Test-Signature FOUND`, so a green `scan_failed` cannot come from a scanner that refused everything. The control 4.12's gauntlet needed, one layer down.
