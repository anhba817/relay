...
- [ ] T020a **THE SWEEP'S BATCH QUERY IS A SORT, AND `data-model.md` §5 REASONED ABOUT THE PREDICATE.** Add the partial index to `services/api/migrations/0018_media_states.sql`: `(created_at) WHERE state = 'pending'`. Measured at analysis pass 2 over 3,028 rows — `Seq Scan` plus a top-N heapsort at **90 buffers and 2.370 ms** for a 50-row batch, against an `Index Scan` at **4 buffers and 0.029 ms**, 88 kB. **Publish it beside 4.12's GIN**: that one is 1.62% of its table and stays there; this one is 11.96% today because every row is `pending` and shrinks to the size of the backlog as objects resolve. Chapter 4.1's *"the sort is 656 of a 698 ms plan"* is the sentence that applies.
