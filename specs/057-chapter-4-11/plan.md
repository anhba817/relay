...
| **VI · Requirement-driven, test-verified** | FR-MED-06 is a `T` clause. **One of its arms cannot be tested** and the chapter says so with the database's own refusal rather than a skipped test (research R2). **AND THE CLAUSE THIS ROW WALKED PAST FOR SIX PASSES**: VI also says *"tenant isolation MUST have 100% branch coverage (NFR-MNT-02)"*, and this chapter's whole mechanism is a tenant-isolation predicate. It is a SQL `WHERE` — **no JavaScript branches to cover**, 048's finding about a sorting key arriving at the same place from a different direction — while the JS around it does have arms, and they land in `repository.ts` at **branches 92** against 92.66 measured, where an uncovered isolation arm passes the ratchet. FR-023 and SC-011 make the form and the figure a deliverable; T050a decides between stating it met and extracting the decision. **This table has now been wrong twice — principle II at pass 3, principle VI at pass 7** — which is the argument for reading the constitution against the built thing rather than filling the row when the plan is written. |
