...
- [X] T068 Write `baseline.txt` carrying every phase's measurements in the order they were taken, and the pinned lane environment.
- [X] T068a **Re-measure the dependency count and compare it to T003's opening figure** (SC-010). T003 records 29 at `part4-ch10`; a criterion measured once is asserted rather than compared, which is the thing T003's own wording says it exists to prevent.
- [X] T069 Write `gaps.md`. **Re-measure every carried item rather than copying it**: 056-1 and 056-2 (this chapter inherits both), 056-9 and 056-10 (closed in the 056 follow-up — confirm they stayed closed), and 055-3 (`check:errors` still has no CI job, and this chapter both removes a code and adds one).
...
- [X] T070b **Amend `docs/07-tutorial-plan.md`'s Part 4 section, which carries a second copy of the same table and has never been amended by any chapter** (FR-017). Four things, and only the first is this chapter's own:
...
- [X] T071 Write `traceability.md`, and **name anything discharged in a weaker form than its words suggest.** FR-010 is the first candidate: the predicate is built and half of it cannot run.
...
- [X] T072 Update `CLAUDE.md`'s `<!-- SPECKIT -->` block for the close, including every task premise this chapter falsified by running it.
- [X] T073 Run the tutorial job's six gates, named from `ci.yml` rather than memory, and **read each one's counted success line**. `lint` is the one with no counted line at all — eslint prints nothing on success.
