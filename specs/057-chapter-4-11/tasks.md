...
- [ ] T005 **Count the fenced files this chapter will touch, against the checker.** The plan listed six as a starting point and says so; 056's table said twelve and the checker said seventeen. `pnpm check:fences --dump` after the first source edit is what answers it. **The plan's list is at eighteen and has grown at five of the six analysis passes** — every growth a door nothing had named, never a miscount of a known file. Expect the checker to add more, not fewer.
...
- [ ] T017h **Budget T017f's and T017g's four files, which are the most expensive the chapter has taken on**: `packages/protocol/src/frames.ts` **10**, `services/gateway/src/fanout.itest.ts` **10**, `services/gateway/src/fanout.ts` **6**, `packages/protocol/src/revision.ts` **2** — 28 titled fences across the two locales, on top of `internal.ts`'s **26**, which T005 carried as `?` and this pass counted. None of the four appears in any artifact before analysis pass 6, because none mentioned the fanout consumer. `api-client.ts` carries 12 and is **not** edited: it passes the schema and the schema is what changes.
