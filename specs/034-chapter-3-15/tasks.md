...
- [ ] T065 [P] [US6] Add `role` (FR-011) to the schema in `services/api/src/channels/channels.schema.ts` as a three-value enum
- [ ] T066 [P] [US6] Test the round trip for all three roles, and a fourth value refused 400 with `field` naming `role` (SC-009). The field name is in the envelope only because chapter 3.14 stopped `ZodValidationPipe` discarding `issues[0].path` — `services/api/src/channels/channels.itest.ts`
...
- [ ] T138 [P] [US8] Test 100 accepted and 101 refused 400 with `field: "users"` (SC-012) — `services/api/src/users/users.itest.ts`
...
- [ ] T159 [P] Test FR-039b: authentication and membership converge on one row for one external identifier, and two rows are never created. Chapter 3.13 made `createUser` idempotent on `(environment_id, external_id)`, which is the property this relies on — `services/api/src/auth/credentials.itest.ts`
