...
- [ ] T013 Generate migration `services/api/migrations/0011_activity_and_read_positions.sql` from the schema: `channels.last_activity_at timestamptz not null default now()`, `index (environment_id, last_activity_at desc)`, and the `read_positions` table per `data-model.md`
- [ ] T014 Add `readPositions` to `services/api/src/db/schema.ts` — `environment_id`, `channel_id`, `user_id`, `sequence`, `updated_at`, primary key `(channel_id, user_id)`. **Carries `environment_id` although `channel_id` determines it**, because feature 030's guard watches tables that have one and `members` is the counter-example that proves the point: it has none, `tenant-scope.itest.ts` classifies it `hop`, and no trigger protects it
- [ ] T015 [P] Add `lastActivityAt` to the `channels` table in the same file, with the index — `services/api/src/db/schema.ts`
- [ ] T016 Generate migration `services/api/migrations/0012_member_roles_and_user_deletion.sql`: `members.role text not null default 'member'` with `CHECK (role IN ('owner','moderator','member'))`, and `users.deleted_at timestamptz`
...
- [ ] T029 Add `read_positions` to the expected classification in `services/api/src/isolation/tenant-scope.itest.ts` — `direct`, because it carries `environment_id` — and confirm the totality assertion still holds over the live catalogue
- [ ] T030 Commit Phase 2
