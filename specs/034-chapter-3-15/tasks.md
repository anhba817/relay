...
| **3.16** what a user sees | 11–17, page at 19–20 | listing, unread, the user surface, implicit creation, and FR-038a's citation class (Phase 17). **24 files** per R18's table, seven of them diffs. `sentinel.sql`, `sentinel.ts`, `guard.itest.ts`, `internal/session.controller.ts`, `scripts/backfill-channel-activity.mjs` and `api-client.ts` are its files, not 3.15's — the middle two were in no bucket until T091 and the last until Phase 15 |
