...
- [ ] T035 [P] [US1] **Write out the send call graph** into `specs/034-chapter-3-15/baseline.txt` rather than quoting a count: `repo.sendMessage` has one non-test caller, `messages.service.ts:47`, plus `isolation/fixtures.ts`; `MessagesService.send` has two, `messages.controller.ts:40` and `internal.controller.ts:65`. This task said "six callers" until pass seven counted them, and the count was hiding T031a — the caller that matters supplied no user, so it inherited nothing. Confirm no caller carries a membership check of its own
