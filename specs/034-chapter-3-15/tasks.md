...
- [ ] T063 [US6] Add `setMemberRole` to `services/api/src/db/repository.ts`
- [ ] T064 [US6] Add `PATCH /v1/channels/:externalId/members/:userExternalId` in the channels controller, per `contracts/membership.md`
- [ ] T064a [US6] **Accept an optional `role` per entry on `POST /v1/channels/:channelId/members`** (FR-011b), defaulting to `member` — `services/api/src/channels/channels.controller.ts` and `channels.service.ts`. US6's first scenario is a member *added with* a role; the plan had add assign the default and `PATCH` change it, which is two calls for one intention and a window where the member holds a role nobody chose
- [ ] T065 [P] [US6] Add `role` (FR-011, FR-011a) to the schema in `services/api/src/channels/channels.schema.ts` as a three-value enum
