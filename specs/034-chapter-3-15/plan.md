...
├── spec.md                  58 FRs, 22 SCs, 9 stories, 42 scenarios, 10 edge cases
├── research.md              R1–R18, deduplicated in analysis pass twelve
...
| `GET /v1/channels/:channelId` | FR-CHN-01 defines a channel's four elements and nothing could read them back. Three artifacts assumed the route existed — SC-001's verb list, FR-003's "every read", the authorization table — and no task built it (FR-003a, analysis pass three). | One handler, one service method, one classification entry, and a fourteenth route in SC-014's count. |
| Bulk removal replacing a singular route | FR-006 says "up to 100 in one request" and FR-007 says "per user"; the contract had specified a single-user `DELETE` for ten analysis passes. Mirroring chapter 3.13's add is the smaller change than leaving a requirement uncovered. | An action-style `POST …/members/remove` with a per-entry result array, and the singular route retired rather than kept beside it. |
| **Attributing the user on the public send and history routes** | Half of chapter 3.14's gap G1, which **R16 first declared out of scope**. FR-001 cannot hold without it: `MessagesController` declares no `@Accepts`, so a user token is accepted, and both handlers called through to a repository function with no caller — so the membership check could not fire on either route (analysis passes seven and eight). | Two handlers resolve the principal, one service threads it, `listMessages` gains a `userId?` parameter. The fan-out half of G1 stays with FR-RTM-05. **This is a scope commitment revised after the plan was written**, which is what this table is for. |
