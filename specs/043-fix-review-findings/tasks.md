...
- [ ] T046 [US4] Write `relay-tutorial/scripts/check-revision-order.mjs` and add it to the `check:docs` script. It costs no amendment: the tutorial's own scripts are fenced by nobody. (FR-021)
- [ ] T047 [US4] Test `relay-tutorial/scripts/check-revision-order.mjs` red by moving one row in `docs/04-srs.md` Appendix D, then restore it. (FR-021)
