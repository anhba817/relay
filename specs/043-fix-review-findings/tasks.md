...
- [X] T018 [US1] Append an amendment hunk to `relay-tutorial/fences/post-series.md` for each **fenced** file changed in this phase. **Fourteen of the fifteen this feature edits take one; `limits.itest.ts` does not** — its chain is excerpt-only, so there is nothing to amend and nothing to catch a mistake in it either. **Run `check:fences` after each one, not once at the end** — it reports only the first difference per file, so "three problems" can be thirty stale lines.
- [X] T018a [US1] Sweep the published chapters for prose this phase falsifies and amend each hit (FR-027). **No gate can find these**: `grep -rn '4100\|4101\|4102\|200 ms\|200ms' 'relay-tutorial/app/(en)/part-*/*/*/page.mdx'` returns hits in six pages, and the two that matter are not fences of any file. `part-2/chapter-08/…/page.mdx:908` publishes an **untitled** ` ```text ` transcript — `api up on 4100`, `gateway 1 up on 4101`, `ws://127.0.0.1:4102` — as the milestone's proof the journey runs, and an untitled fence names no file so `check-fence-chain` skips it. `part-3/chapter-13/…/page.mdx:1542` publishes the port map as a text block, under a sentence warning that a range *"goes stale the next time a file is added"* — **and line 1509 publishes it again as an `(excerpt)` fence**. **Amend both.** An excerpt fence falls between this feature's two mechanisms: a prose sweep does not think to look inside a fence, and the chain check skips anything titled `(excerpt)`. **Mirror every amendment into the Vietnamese page.**
- [X] T019b [US1] Move the remaining eight spawn helpers off hand-allocated port bands to `PORT=0`, reading the assignment from the child's own `listening` line: `services/gateway/src/{membership,limits,presence,meter,public-surface,isolation,session}.itest.ts` and `services/dispatcher/src/dispatcher.itest.ts`. **The e2e harness was one of nine, not the only one.** Two of the nine bands contained a service the lane itself runs — `membership` drew 5400-5599 which holds Postgres's 5432, and `limits` drew 4100-4299 which holds NATS's 4222. Delete the port map in `limits.itest.ts` rather than correcting it: a hand-maintained table cannot know which services a contributor runs. (FR-001, FR-002, FR-026)
- [X] T019c [US1] Fix `membership.itest.ts` and `presence.itest.ts`'s health probes, which fetch `/health` — a route this api has never served (`services/api/src/health.controller.ts:7` answers `/healthz`). Both loops ran a hundred failed probes and then returned the url anyway, so each was a flat ten-second sleep reporting success. **Make the loop end in a throw**; neither fault could surface while the other was there to absorb it. (FR-002)
- [X] T019d [US1] Append an amendment hunk to `relay-tutorial/fences/post-series.md` for each file T019b and T019c changed, and re-run `check:fences`. (FR-027)
- [X] T019e [US1] Stop `services/gateway/src/limits.itest.ts`'s two counting tests straddling a wall-clock minute. `limits.ts:115` keys the budget on `Math.floor(now / 60_000) * 60_000`, so ten sends either side of the boundary write two buckets and read one — reported as `expected 7 to be 10`, which reads exactly like a limiter losing increments. **Sleep to the next boundary rather than relaxing the assertion**: `toBe(10)` is the point of the test. Found by run 1 of the second battery; latent before this feature, and surfaced because retiring the port bands shortened the suite from 45.7 s to 35.3 s and moved its phase against the clock. (FR-006)
- [X] T019 [US1] Verify: `node relay-platform/scripts/reset-lane.mjs`, remove the sequencer caches, then twenty consecutive `pnpm test:integration`. Then `docker compose down` and `pnpm -s test`. Record both, and record which durable consumers outlive one run — **service-created durables are not test debris and are counted separately** (SC-003). (FR-006, SC-001, SC-002)
- [X] T019a [US1] Re-measure the e2e package's duration and the integration total against T006a, and settle SC-012. If the total exceeds 240 s, **record which of three was chosen and why**: raise the budget with this measurement attached, make `harness.itest.ts` boot once instead of twice, or bound the teardown wait lower. **Do not let the budget quietly become whatever the lane now costs** — a budget that follows the measurement is not a budget, and this feature's own fixes are what spend it.
...
- [X] T020 [US2] Export `MESSAGE_TEXT_MAX` from `relay-platform/packages/protocol/src/frames.ts`, beside `messageSchema`. **Not from `attachments.ts`**, whose six exports are all about attachments — a message-text bound on that shelf is the drift this task exists to remove. `internal.ts` already imports `messageSchema` from `./frames.js`, so the direction exists and no cycle is created. The value is 8000, which is what the REST and internal doors already carry. (FR-008)
- [X] T021 [US2] Apply it in `packages/protocol/src/frames.ts` at **`messageSendSchema.payload.text`**, not at `messageSchema.text`. **This task named the wrong one of two unbounded `text: z.string()` declarations in that file.** `:34` is `messageSchema` — the OUTBOUND message the server emits, and the schema that carries a tombstone (`:118`). Bounding it would tighten a READER of stored rows, which is chapter 3.24's `outboxEventSchema` defect exactly. The socket door with no bound is `messageSendSchema` at `:72`. (FR-008)
- [X] T022 [P] [US2] Import it in `packages/protocol/src/internal.ts:32`, replacing the literal. (FR-008)
...
- [X] T024 [US2] **Already satisfied — verified, not changed.** `session.ts:1452-1464` answers a failed `frameSchema.safeParse` with the registered code `invalid_frame` AND `issues[0].path.join(".")` as `field`, and returns before `api.sendMessage`. Bounding `messageSendSchema.payload.text` gets FR-009 and FR-010 for free. The code choice is settled by precedent at `frames.ts:80-88`, not preference: one payload must not be refused at two layers under two codes. Red probe confirms it — removing the bound yields `expected 'invalid_request' to be 'invalid_frame'`, the review's finding reproduced. Original text: make the gateway's refusal carry a registered code and `field`, in `relay-platform/services/gateway/src/session.ts`. `sendError` has taken an optional `field` since chapter 3.24; no new plumbing is needed. (FR-009, FR-010)
- [X] T025 [US2] Build `relay-platform/packages/protocol` so its `dist` carries the new export from `packages/protocol/src/frames.ts`, then type-check the api and the gateway against it. **Consumers resolve `@relay/protocol` through `dist`**, so a protocol change is invisible until it is built — and the compiler's list of call sites is the inventory, not a grep. (FR-008)
- [X] T026 [US2] Write the tests in `relay-platform/packages/protocol/src/frames.test.ts`, `services/api/src/messages/messages.itest.ts` and `services/gateway/src/session.itest.ts`: the same over-long text refused at all three doors, and the socket refusal naming `payload.text` and making no internal request. **Assert the absence of the internal request**, not only the refusal — a test that checks the refusal alone passes whether or not the hop was saved. (FR-010, SC-004)
...
- [X] T028 [P] [US2] Measure and record in `baseline.txt` how many stored `avatar_url` values the new rule would reject (FR-013). Research measured zero in this database. **Record it as a measurement of this database**, not as a claim about a customer's.
- [X] T029 [US2] Write the tests in `relay-platform/services/api/src/users/users.itest.ts`: `javascript:`, `data:`, `file:` and `vbscript:` each refused with `field: "avatar_url"`, and `https:` accepted. (SC-005)
- [X] T030 [US2] Append an amendment hunk to `relay-tutorial/fences/post-series.md` for each file changed in this phase, and re-run `check:fences` after each. **Then sweep this phase's published prose (FR-027)**: `avatar_url` appears in five chapter pages and the message-length rule in more, and no gate reads any of it.
- [X] T031 [US2] Verify against `quickstart.md` scenarios 2 and 3, including the grep that proves the maximum is defined once. (SC-004)
...
- [X] T032 [US3] Add `WEBHOOK_EVENT_TYPES` — the eight types FR-WHK-02 declares, each marked with whether the platform emits it. **Derive `OUTBOX_EVENT_TYPES` from it rather than maintaining both.** Two lists that must agree and are maintained separately is what `gaps.md` 3.23-4 records about `targets.ts`, and what `eslint.config.mjs`'s own comment says *MUST AGREE* with nothing comparing them. (FR-016)
- [X] T033 [US3] Make adding a type to `WEBHOOK_EVENT_TYPES` without deciding `emitted` a compile error, and add a test asserting the exact declared set and its count. Chapter 3.24 added two error codes where its plan expected one, and `codes.test.ts`'s exact-count assertion is what caught it. (FR-016)
- [X] T034 [US3] Add one error code per customer-caused webhook refusal to `packages/protocol/src/codes.ts`. **Read the filter's ladder before choosing statuses**: `ProtocolErrorFilter` derives a code from 400, 401, 403 and 404 and answers `internal_error` for anything else, which is why a bare 422 misattributes the fault. (FR-014)
- [X] T035 [US3] Replace the five bare `UnprocessableEntityException` throws in `relay-platform/services/api/src/webhooks/webhooks.service.ts` at lines 88, 193, 196, 202 and 210 with `protocolError(code, message, 422)`, following chapter 3.24's `media_not_available` precedent. (FR-014)
- [X] T036 [US3] Change `assertEventTypes` at `webhooks.service.ts:208` to validate against the **declared** eight. **Not against `OUTBOX_EVENT_TYPES`** — the review and `gaps.md` 3.23-1 both recommend that and both are wrong: 741 stored subscriptions name `channel.created`, which FR-WHK-02 declares and the platform does not emit yet. Accept those and say the type is not emitted yet. (FR-016)
- [X] T037 [P] [US3] Re-run the event-type query and record the count in `baseline.txt` (FR-017), including how many name a declared-but-unemitted type and how many name nothing at all.
- [X] T038 [US3] Write a section in `docs/08-error-reference.md` for each new code, reachable from its `docs_url`. Constitution V and NFR-USE-05 both require it and `check:errors` enforces it. (FR-015)
- [X] T039 [US3] Document all six close codes in `docs/08-error-reference.md`. **Inside the `**Status:**` line of the error code that carries each one** — chapter 3.21's convention. A `## 4001` heading fails `check-error-codes.mjs:59`'s orphan check with no exemption. (FR-018)
- [X] T040 [US3] Extend `relay-tutorial/scripts/check-error-codes.mjs` to compare `CLOSE_CODES` against the reference's text as it already does for `ERROR_CODES`. (FR-019)
- [X] T041 [US3] **All three done** — removing a close code's text and renaming one both fire, and the rename trips BOTH directions at once (4003 undocumented, 4007 invented). The third adds a close code to `packages/protocol/src/codes.ts`, which needs a rebuild of the shared `dist` the running battery's api children load, so it waits for a free lane. Test `relay-tutorial/scripts/check-error-codes.mjs` red, three ways: remove one close code's text from `docs/08-error-reference.md`, add a close code to `packages/protocol/src/codes.ts` with no text, and rename one. **A checker that has never failed has an unverified class list.** (FR-019)
- [X] T042 [US3] Write the route tests: each of the five refusals asserted **by code**, and a misspelled event type refused while `channel.created` is accepted with the not-emitted-yet answer. Today `webhooks.itest.ts:90` asserts the status and the message text, which is why nothing has caught this since chapter 3.5. (SC-006, SC-008)
- [X] T043 [US3] Append an amendment hunk to `relay-tutorial/fences/post-series.md` for each file changed in this phase, and re-run `check:fences` after each. **Then sweep this phase's published prose (FR-027)**: `OUTBOX_EVENT_TYPES` appears in three chapter pages, and this phase changes what subscriptions are validated against.
- [X] T044 [US3] Verify against `quickstart.md` scenarios 4, 5 and 6. **Scenario 5's second half was unmet and is now built**: the contract requires a declared-but-unemitted type to be accepted *and said so*, and the first implementation accepted it silently — which is the same permanently-silent endpoint the finding is about, for a blameless reason. The 201 now carries `not_emitted_yet`, present only when non-empty. (SC-007)
...
- [X] T045 [US4] Reorder `docs/04-srs.md` Appendix D so the version column ascends. It reads 1.0, 1.1, 1.2, 1.3, 1.4, **1.7, 1.6, 1.5** — three chapters each inserted above their predecessor. (FR-020)
- [X] T046 [US4] Write `relay-tutorial/scripts/check-revision-order.mjs` for `docs/04-srs.md` and add it to the `check:docs` script. It costs no amendment: the tutorial's own scripts are fenced by nobody. (FR-021)
- [X] T046a [US4] Add the same rule for this feature's own success criteria to `specs/043-fix-review-findings/check-refs.py`, which already reads `spec.md`. **Not to the tutorial's script** — `relay-tutorial` is its own git repository, nothing in it reads `../specs`, and `sync-docs.sh`'s precedent for `../docs` errors out when the parent is absent. A feature directory is transient; a gate pointed at one dies with it. (FR-021a)
- [X] T047 [US4] Test both gates red: move one row in `docs/04-srs.md` Appendix D, and move one success criterion in `specs/043-fix-review-findings/spec.md`. Restore both. (FR-021, FR-021a)
- [X] T048 [P] [US4] Correct the bot/quota finding in `docs/09-platform-implementation-review-2026-09-03.md` to state what `assertWithinQuota` does: the message hard cap throws before the sender test, and the exemption is from the unique-active-persons ceiling alone (FR-022). (SC-010)
...
- [X] T050 [US4] Retire the migration generator: delete `relay-platform/services/api/migrations/meta/`, remove the `generate` step, and state in `drizzle.config.ts` that migrations here are hand-written and reviewed against SAD §6.1. **Add an assertion that fails if `migrations/meta/` or the generate script returns** (FR-023, FR-023a) — a decision recorded only in a comment is a decision the next contributor reverses by accident. The constitution already says this — *"migrations remain versioned, forward-only, hand-reviewed SQL"* — so the tooling has contradicted it since chapter 3.9, not the other way round.
- [X] T051 [US4] Amend FR-RTM-10 in `docs/04-srs.md` to state the bound that applies when the fabric is unavailable, citing ADR-20, which already says *"exceeding the clause by 55 seconds"* and gives the arithmetic. (FR-025, FR-025b)
- [X] T052 [US4] Amend FR-RTM-09 in `docs/04-srs.md` to state that the connection limit is best-effort and what happens when it cannot be checked, citing ADR-23. (FR-025a, FR-025b)
- [X] T053 [US4] Read both amended clauses in `docs/04-srs.md` against the measured behaviour and confirm neither permits more than the platform already does (FR-025c). **This is the one check no instrument can run**: "amend the clause" and "weaken the clause" are one edit apart, and every gate here compares bytes. (SC-011)
- [X] T054 [US4] Add revision **1.8** to `docs/04-srs.md` Appendix D for this feature's two clause amendments — **appended, below 1.7**, which is the shape T046's gate now enforces. (FR-020)

- [X] T050a [US4] Teach `relay-tutorial/scripts/check-fence-chain.mjs` to retire a file from a post-series fence. `replay` has understood `path (deleted)` since chapter 3.2 retired `environment-context.guard.ts`; the appendix loop never did, so it could amend any published file and delete none — and once Part 3 closed, the appendix is the only place a change can land. Found by T050, whose `drizzle.config.ts` deletion the checker answered with "must be a diff", which is true of an amendment and not of a deletion. Test it red both ways. (FR-027)
- [X] T050b [US4] Append T050's amendment hunks and the `drizzle.config.ts` retirement to `relay-tutorial/fences/post-series.md`, and re-run `check:fences`. (FR-027)
