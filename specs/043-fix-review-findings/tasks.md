...
- [ ] T041 [US3] **Two of three done** — removing a close code's text and renaming one both fire, and the rename trips BOTH directions at once (4003 undocumented, 4007 invented). The third adds a close code to `packages/protocol/src/codes.ts`, which needs a rebuild of the shared `dist` the running battery's api children load, so it waits for a free lane. Test `relay-tutorial/scripts/check-error-codes.mjs` red, three ways: remove one close code's text from `docs/08-error-reference.md`, add a close code to `packages/protocol/src/codes.ts` with no text, and rename one. **A checker that has never failed has an unverified class list.** (FR-019)
