...
# DERIVED, NOT WRITTEN DOWN (analysis pass 9). This read `CHAPTER = "3.24"` — the
# PREDECESSOR's number, carried in by the copy-forward and invisible to anyone reading
# the file. `check-refs.py` beside it derives the same value from the directory name and
# always did. A feature whose directory names no chapter gets `?.?`, and `verifiers()`
# then matches the bare id.
_m = re.search(r"chapter-(\d+)-(\d+)$", pathlib.Path(__file__).resolve().parent.name)
CHAPTER = f"{_m.group(1)}.{_m.group(2)}" if _m else "?.?"


def verifiers(ident: str, task_lines: list[str]) -> list[str]:
    # A NON-CHAPTER FEATURE HAS NO SUFFIX TO MATCH (analysis pass 9). The `(3.24)`
    # suffix disambiguates ids that collide ACROSS chapters; inside one feature's own
    # tasks.md there is nothing to disambiguate, and `043-fix-review-findings` yields
    # CHAPTER "?.?", which matches nothing at all — every id read as untraced.
    pat = re.escape(ident) + (r"\b(?! \()" if CHAPTER == "?.?"
