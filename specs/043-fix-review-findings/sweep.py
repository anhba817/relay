#!/usr/bin/env python3
"""Cross-artifact consistency sweep for chapter 3.24.
...
SKIP = (
    # Inherited from chapter 3.21 through 3.22 and kept, because the record still quotes it.
...
def main():
    ...
    # Chapter 3.21's plan wrote `### Phase N — title`; chapters 3.22 and 3.23 write
    ...
    measured = {
        ...
        # `[a-z]?` ADDED IN CHAPTER 3.23, and the sibling instrument had the
        # identical hole. `SC-002a` and `SC-006a` were invisible, so this printed
        # eleven where the file declares thirteen — a wrong number presented as a
        # fact, which is worse than no number.
        "success criteria": len(set(re.findall(r"\*\*(SC-\d+[a-z]?)\*\*:", docs["spec.md"]))),
        "phases": len(tasks),
    }
