...
- [ ] T054 [P] Run `pnpm check:fences`, `pnpm check:figures`, `pnpm check:errors`, `pnpm check:srs`, `pnpm check:docs`. **Two of these can be green for the wrong reason**: `check:errors` reads `packages/protocol/dist/codes.js`, so build first; `check:docs` exits 0 when the parent `docs/` is absent (T050b), so confirm it ran
