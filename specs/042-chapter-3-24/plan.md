...
| A **required** field on `messageSchema`, not an optional one | FR-022 (3.24): a reader should need no special case on the wire either, and a `strictObject` makes the compiler name every producer that forgets. **The cost, counted rather than described:** 15 files import the `Message` type and **8 build a literal — 18 in all** — so `typecheck` is red from the moment the field lands until every one is set. That is why the task that sets the field everywhere sits in **Phase 3, beside the one that makes it required**, and not two phases later — the first draft put it in Phase 5 and would have committed two phases with a broken typecheck. It also costs a rolling deploy: mid-deploy an old instance publishes without the field and a new instance drops the frame, which ADR-07 permits because the cursor recovers. | An optional field, which lets the wire omit the key on the one path a client cannot re-read |
