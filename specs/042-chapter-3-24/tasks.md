...
- [ ] T027a [US1] Find every remaining place that constructs a `messageSchema` payload and set the field, using the compiler rather than a reading: `pnpm -s typecheck` after T014 lists them. Research R3 counted **five construction points** across `messages.controller.ts`, `session.ts` and `backfill.controller.ts`; **treat that number as a prediction to check, not a list to work from** — chapter 3.23 re-derived a count of typed points and found seven where its record said three.
