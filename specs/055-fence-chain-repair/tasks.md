...
- [X] T001 **DONE — 110, APPLY 74, HEAD 36, exactly as expected.** Pinned in `baseline.txt` with all three repository heads and the fact that both submodules were clean. Original: Run `pnpm check:fences` from `relay-tutorial` and record the total, APPLY and HEAD in `specs/055-fence-chain-repair/baseline.txt`. Expected 110 — APPLY 74, HEAD 36. **Any divergence from that is the first finding** (FR-013), not a rounding.
- [X] T002 [P] **DONE — 42 / 32 / 25 / 11, exact.** Original: Classify all problems into the four classes and record each count in `baseline.txt`: `matched 0 times`, `no earlier fence to amend`, `differs at line`, `does not exist in relay-platform`. Expected 42 / 32 / 25 / 11.
- [X] T003 [P] **DONE — 36 HEAD en · 30 APPLY en · 30 APPLY vi · 14 APPLY appendix, and 0 against `relay-platform`.** `baseline.txt` also carries the class-by-locale matrix analysis pass 4 added. Original: Record where each problem lives — `app/(en)`, `app/(vi)`, `fences/post-series.md` — and confirm **zero are reported against `relay-platform`**. Expected 36 HEAD en · 30 APPLY en · 30 APPLY vi · 14 APPLY appendix.
- [X] T004 [P] **DONE — 47 targets, 36 platform files and 11 phrases, top eleven holding 64 of the 110.** Exact. Original: Attribute every problem to the target it is about, by the message where it names one and by the fence's title where it does not, and record the per-target counts in `baseline.txt`. Expected **47 targets** — 36 platform files, 11 phrases — with the top eleven holding 64 of the 110.
- [X] T005 [P] **DONE — 30 vi problems, all 30 with an exact en twin at the same chapter, line and message; 0 vi-only.** The 36 en problems with no vi twin are the HEAD class, which the vi chain is never compared for (050-3). Original: Confirm the en/vi mirror: every vi problem has an en twin at the same chapter slug, same line, same message, and **zero are vi-only**. This is what makes the diagnosis half the size of the count, so it is checked rather than assumed.
- [X] T006 [P] **DONE — `MIRROR` 0.** Original: Record `MIRROR` at **0** in `baseline.txt`. It is the check that every locale repair is a faithful copy, and it must read 0 at every later measurement.
...
- [X] T007 [P] **DONE — 2,109 / 1,749 / 360 / 222, exact, with the locale split recorded: en 1,071 · vi 1,005 · appendix 33, and the 222 as 111 and 111.** `NOT_A_FILE` skips 224, the extra pair being `.naive.`. Original: Measure the unread-fence population and record it: opening fences, titled, untitled-with-a-language, and titles already carrying `(excerpt)`. Expected 2,109 / 1,749 / 360 / 222. **`gaps.md` 043-1's "146 of 904" is a 2026-08 figure** and this is the number SC-008 is measured against.
...
- [X] T008 **DONE, AND RESOLVED RATHER THAN RECORDED — THE FILE IS `.gitignore` AND THE CAUSE IS NOT THE TUTORIAL.** 046's T006 blamed a `relay-tutorial` commit; all six tutorial commits in that window are Vietnamese translations, and a vi commit cannot add a HEAD problem. The cause is `relay-platform`'s `ea0cb513 chore(046): the seeder's volumes`, which added `corpus.json` to `.gitignore` — chapter 1.1 publishes that file whole at 7 lines, the tree became 8, HEAD 35 -> 36. **So the 110th problem is feature 046's own, made during the same feature whose opening measurement first reported it**, and the delta-of-0 convention hid it because it compares a total to a total and never asks which file. Original: Resolve or record the **109 → 110** discrepancy (FR-014): `specs/045-part-3-rework/gaps.md:3327` reports 109 — APPLY 74, HEAD 35, and feature 046's T062 measured 110 — APPLY 74, HEAD 36 and reported it as *"a delta of 0"* against its own opening. One HEAD problem appeared in between. Name the file if the per-file lists can be recovered from either feature's record; record the mechanism either way, because **the delta-of-0 convention is what kept it invisible for nine chapters**.
- [X] T009 **DONE.** Commit phase 1.
...
- [X] T011 **DONE — AND BOTH HALVES OF THE CONTROL RAN, IN BOTH LOCALES.** Verdict unchanged (110/74/36 with the flag, without it and with the `--`); two dumps `diff -r` byte-identical. The positive control: chapter 3.14's working hunk for `users.controller.ts` occurs **1 time** in the `--at` state and **0 times** in the final state, which proves the modes are different states and that `--at` stops before the page's own fences. The vi control: `--at` on a vi page writes 232 paths and no `reconcile.ts`; `--at` on en 4.9 writes 286 and does. Trailing newline checked by dumping two undiverged paths and diffing against the tree — zero bytes each. **`reconcile.ts` is chained in en 4.7 as well as 4.9**, which this task said was 4.9 alone; the control is unaffected, because it is in no vi chapter at all. Original: Prove the dump is the same replay the check uses: dump twice and `diff -r` the trees; run `pnpm check:fences` with and without the flag and confirm the same count. **A generator that replays differently produces hunks the checker rejects for reasons neither of them explains.**
...
- [X] T012 **DONE — 285 printed, 285 on disk, and `--at` 3.14 writes 179.** **AND THE APPENDIX RETIRES A PATH, WHICH NO ARTIFACT SAID.** The pre-4.9 state holds **286** where the end holds 285; the extra is `services/api/drizzle.config.ts`, retired by `fences/post-series.md:2500`. `data-model.md`'s *"it can amend a path and never introduce one"* is true and incomplete — the appendix can also remove one, which is why a later `--at` state can be larger than the final. The 285/276-against-10 gap is unresolved and carried to T095. Original: Confirm the dump covers every chained path: `find <dir> -type f | wc -l` against the count `--dump` prints for itself. Expected **285 now** for the English final-state mode; the vi chain holds **276**. **`--at <page>` writes fewer** — only the paths the chain has seen by that page, which for an early chapter is a small fraction of 285 — so its count is recorded per chapter rather than compared against a fixed number.
...
- [X] T013 **DONE — 318 lines against 1,182, and zero `env:` blocks where the tree has one.** The hunk that would create it is `+    env: {` at `fences/post-series.md:1524`, one of the fourteen that no longer apply, so the nine anchored inside it have nothing to attach to. Recorded in `baseline.txt`. Original: Dump the state for `vitest.coverage.config.mts` and record in `baseline.txt` that it is **318 lines against the tree's 1,182 and contains no `env` block**, together with the dump's own file count at this point (285, before phase 3 takes it to 274). This is the measurement that explains nine of that file's fifteen bad hunks, and it is the instrument's own positive control — if the dump does not show it, the dump is wrong.
- [X] T014 **DONE — `pnpm lint` exit 0, `pnpm build` exit 0 with every page rendered.** `npx prettier --check` warns on the edited script and warned on it before the edit too (checked against `git show HEAD:`); it is not a gate in the tutorial job, so the file keeps its own style. Original: Run lint and `pnpm build` in `relay-tutorial`; commit phase 2.
...
- [X] T029 [US1] **DONE — and it measured what the obvious method costs.** `diff(chain state at 3.17, rework/part3-ch17)` is **206 changed lines in one hunk**, because the chain was already 148 lines behind ch16 before the chapter started. Re-anchoring the chapter's own change instead gave **73 body lines — the same size as the published hunk** — and 99 → 97. The replay order was exactly as this task predicted. Original: `vitest.coverage.config.mts` — **15 of the 42**, and the largest single target in the feature. Repair the **first** failing hunk only, then re-measure before touching the other fourteen. **The first is `app/(en)/part-3/chapter-17/the-words-somebody-wants-back/page.mdx:6703`**, measured in replay order:
...
- [X] T030 [US1] **DONE — 15 of 15, 97 → 84, and EIGHT WERE SHADOWS.** Seven repair operations cleared fifteen problems; one appendix hunk (`:1730`) cleared five at once, and repairing 3.22 made 3.23 apply on its own. Original: `vitest.coverage.config.mts` — repair whatever of the fifteen survives T029, in chapter order, re-measuring after each. Record how many were shadows, because **that ratio is the feature's headline finding about what a count of 110 means**.
- [X] T031 [US1] **DONE — 6 of 6, and one needed a judgement.** The 3.23 hunk adds `RELAY_QUOTA_RELAY` after `RELAY_NOTIFICATION_RELAY`, which the chain does not carry; re-anchored on the chain's actual last entry, `RELAY_NATS_REPLICAS`. Original: `turbo.json` — 6 bad hunks across 3.22, 3.23, 3.24 (3 en + 3 vi). Fenced by 10 chapters plus 1 appendix hunk, so regenerate rather than republish.
- [X] T032 [US1] **DONE — 4 of 4, 82 → 80, both fences mechanical.** Original: `services/api/src/app.module.ts` — 4 bad hunks across 3.22 and 3.23. Fenced by 11 chapters.
- [X] T033 [US1] **DONE — 4 of 4, and one needed a judgement.** `quota_exceeded` goes before `webhook_endpoint_limit_reached` in the tag and the chain has no such key, so it was moved to directly after `channel_member_limit_exceeded` — the key the chapter's own comment argues with. Original: `packages/protocol/src/codes.ts` — 4 bad hunks across 3.23 and 3.24. Fenced by 12 chapters — **seventh-deepest in the series, not the deepest**: `services/api/src/db/repository.ts` is fenced by **24**, `session.ts` by 17, `schema.ts` by 15, `internal.ts` by 14, `session.test.ts` and `messages.service.ts` by 13. Twelve is deep enough for the argument — regenerating an early fence is the 111 → 203 trap — and the superlative was wrong, which matters because it taught a ceiling half the real one. No target in this feature touches `repository.ts`.
- [X] T034 [US1] **DONE — 3 of 3, and the appendix hunk turned out to DUPLICATE the chapter's refactor.** Once 3.25 applied, the appendix hunk had nothing left to delete; regenerated against the chain it went 571 body lines → 122 and closed the file's HEAD divergence with it. 047-1 is closed: the file took a fence. Original: `eslint.config.mjs` — 3 bad hunks in 3.25 and the appendix. `gaps.md` 047-1 recorded this file as unable to take a fence at all; that entry is re-measured here and closed or re-stated with today's numbers.
- [X] T035 [P] [US1] **DONE.** Original: `services/api/src/webhooks/test-event.itest.ts` — 2 bad hunks in 3.22.
- [X] T036 [P] [US1] **DONE — and see T042a: this repair carries FR-016's `END $;` fix.** Original: `packages/test-harness/src/sentinel.sql` — 2 bad hunks in 3.23.
- [X] T037 [P] [US1] **DONE — and it revealed a HEAD divergence that had been invisible behind the APPLY failure.** Original: `packages/e2e/src/harness.ts` — 2 bad hunks in 3.22.
- [X] T038 [P] [US1] **DONE.** Original: `package.json` — 1 bad hunk in the appendix. Chapter 4.9 edited this hunk's `+` side; the pre-image failure predates that.
- [X] T039 [P] [US1] **DONE.** Original: `services/api/package.json` — 1 bad hunk in the appendix.
- [X] T040 [P] [US1] **DONE.** Original: `services/api/src/auth/credentials.itest.ts` — 1 bad hunk in the appendix.
- [X] T041 [P] [US1] **DONE.** Original: `services/gateway/src/presence.itest.ts` — 1 bad hunk in the appendix.
- [X] T042 [US1] **DONE — 99 → 55, per-target, in `baseline.txt`.** Expected 57; two better. **The cascade runs both ways**: 42 bad hunks were cleared by 24 repair operations, and four divergences closed because a failing hunk was their whole cause while **two appeared that were always there** — `turbo.json` and `packages/e2e/src/harness.ts`, whose HEAD divergence was invisible behind an APPLY failure on the same file, because a checker reports the first failure per file. 25 − 4 + 2 = 23. **A count of 110 was neither 110 defects nor an upper bound on them.** No file raised the total. Original: Re-measure and record after every file above, in `baseline.txt`: **the total, APPLY, HEAD and `MIRROR`**, with the file that preceded each number and **the command that regenerated its hunks** (FR-003, FR-011, SC-004, SC-009). A recorded command is what makes the repair reproducible; chapter 4.2 published one nobody ran through five analysis passes.
...
- [X] T042a [US3] **DONE — and no chapter hunk grew.** Published diff-fence body lines **2,379 → 1,882, 497 fewer**; the only growth is in the appendix, which carries no prose obligation. **The teaching is byte-identical in 252 of 261 fences.** Of the three chapter fences that changed, one is the neighbour rename above, one deletes a shorter list, and one is **FR-016's typo**: regenerating 3.23's `sentinel.sql` hunk against the chain produced `-DO $`/`+DO $$` and `-END $;`/`+END $$;`, so a reader copying that listing no longer gets a syntax error — two phases before T066, the task that was going to record it. Five distinct chapters, ten chapter-locale pairs read. Original: **Read the prose beside every hunk this phase regenerated in a chapter** — the 28 of the 42 that are not in `fences/post-series.md` — and confirm it still describes what the fence now shows; record the count of chapters read (SC-007). **A regenerated hunk is not the hunk it replaced.** It is `diff(chain state at that chapter, the file at rework/part3-chN)`, so it absorbs whatever divergence the chain was carrying for that path — which means it can be **larger** than the hunk it replaces and can show the reader lines that chapter never discusses. Record every case where a regenerated hunk grew, with the line counts before and after: that is the phase's prose risk and it has a mechanism behind it rather than being a general caution.
...
- [X] T043 **DONE — `pnpm lint` exit 0, `pnpm build` exit 0.** Original: Run `pnpm build`; commit phase 4.
...
- [X] T044 [US3] **DONE — AND A FOURTH DESIGN BEAT ALL THREE.** One command settled it: **every one of the nine first `diff` fences applies cleanly to its own `rework/part3-chM` tag**, the state BEFORE the chapter's change. So the body goes at chapter N, taken from chM, immediately **before** the existing diff, which is left alone — design A's placement at design B's cost, and the reader meets the file before the change. **Eight bodies, 1,100 lines**, against design A's 1,956 and design B's 1,658. `fanout.itest.ts` is the one declared: **279 lines per problem**, 1.7× the next worst. And the ratio had to be per PROBLEM — per chapter, `session.itest.ts` reads 281 lines for an 8-line diff and would have been declared, but it is amended by five chapters, so one body buys ten problems at 28 lines each. Original: **Decide, per file, between the three designs and record the decision, its reason AND THE TAG IT IMPLIES**, before any body is taken: replace the first `diff` with a whole body at that chapter (`rework/part3-chN`), add the body to an earlier chapter that introduces the file (`rework/part3-chM`), or **declare the file's fences excerpts and record the exception under FR-012** (T056's route, taken as a decision rather than reached as a failure). Nine judgements about published prose; research could not settle them by measurement. **This task blocks T045 through T053** — it is not a task to do beside them.
...
- [X] T045 [US1] **DONE — 281 lines at 3.3 from `rework/part3-ch2`.** Original: `services/gateway/src/session.itest.ts` — first amended in 3.3 and again in 3.5, 3.9, 3.17, 3.22. Ten problems, the second-largest target. **`rework/part3-ch2` 281 lines · `rework/part3-ch3` 287 · today 1,626.** Measured: chapter 3.3's existing hunk applies to ch2 and not to ch3.
- [X] T046 [P] [US1] **DONE — 93 lines at 3.11 from ch10.** Original: `services/api/src/outbox/event.test.ts` — amended in 3.11 and 3.17. 4 problems. **ch10 93 lines · ch11 122 · today 524.**
- [X] T047 [P] [US1] **DONE — 92 lines.** Original: `packages/test-harness/src/driver-exempt.test.ts` — amended in 3.11 and 3.22. 4 problems. **ch10 92 lines · ch11 104 · today 127.**
- [X] T048 [P] [US1] **DONE — 49 lines, the cheapest body in the phase after the dispatcher config.** Original: `packages/protocol/src/internal.test.ts` — amended in 3.20 and 3.24. 4 problems. **ch19 49 lines · ch20 114 · today 253** — the widest gap between the two candidates in the phase, so the design decision costs 65 lines here.
- [X] T049 [P] [US1] **DONE — 102 lines.** Original: `services/api/src/messages/history.itest.ts` — amended in 3.11. 2 problems. **ch10 102 lines · ch11 150 · today 150.**
- [X] T050 [P] [US1] **DONE — 136 lines.** Original: `services/api/src/messages/idempotency.itest.ts` — amended in 3.11. 2 problems. **ch10 136 lines · ch11 144 · today 224.**
- [X] T051 [P] [US1] **NOT A BODY — DECLARED UNDER FR-012.** 558 lines to verify a three-line change is 279 per problem, and the chapter already publishes two `(excerpt)` fences of this file. See T056. Original: `services/api/src/fanout/fanout.itest.ts` — amended in 3.17. 2 problems. **ch16 558 lines · ch17 559 · today 561** — the two candidates differ by one line, and it is still the largest body in the phase.
- [X] T052 [P] [US1] **DONE — 333 lines.** Original: `packages/test-harness/src/guard.itest.ts` — amended in 3.23. 2 problems. **ch22 333 lines · ch23 427 · today 442.**
- [X] T053 [P] [US1] **DONE — 14 lines.** Original: `services/dispatcher/vitest.integration.config.mts` — amended in 3.23. 2 problems, and the cheapest file in the phase. **ch22 14 lines · ch23 49 · today 49.**
- [X] T054 [US1] **DONE, AND IT FIRED.** Four amendments stopped applying once their file was introduced — `session.itest.ts` at 3.17 and 3.22 and `event.test.ts` at 3.17 — which is precisely what this task exists to catch. Every other later amendment applied untouched. The four are T056's. Original: After each introduction, confirm **every existing amendment for that path still applies** — by running the checker, not by reading. An introduction whose content is right for the reader and wrong by one line leaves the later hunks anchored on bytes that do not exist.
- [X] T054a [US1] **DONE — 55 → 37 on the eight bodies, then → 29 on the four declarations, per file in `baseline.txt`.** APPLY reached **0**. The six new HEAD problems are files whose chain now completes and can be compared to the tree for the first time. No file raised the total. Original: Re-measure and record after **every file** in this phase, in `baseline.txt`: the total, APPLY, HEAD and `MIRROR`, with the file that preceded each number and the tag the body came from (FR-003, FR-011, SC-004, SC-009). **Phase 5 had no measurement task until analysis found it missing** — phases 4 and 6 had one and this one did not, which is the shape this feature is about: a check that exists in one place and not its twin. A file that raises the count is reverted or finished before the next is started (FR-004).
- [X] T055 [US3] **DONE — and the eight listings arrived with no prose at all.** Five landed immediately after a closing fence; three sat under prose introducing the diff rather than the body. One sentence written for each, **in English only**: FR-011 forbids altering translated prose, so the Vietnamese chapters carry the listing without an introduction and that is the translator's to write. `MIRROR` is unaffected — it compares fence bodies and the title list, not the prose around them. Four distinct chapters, eight chapter-locale pairs read. Original: **Read each repaired chapter's prose beside its new listing** and confirm the prose describes what the fence shows. Record the count of chapters read. **One of SC-007's three read-throughs** — T042a covers the regenerated hunks and T084a the appended ones; this one covers the introductions, which are the largest listings the feature publishes and the only ones a chapter has never shown before. Feature 045's sentence is the standard: *"putting those into the last chapter that happened to fence the file would make that chapter show a reader code it never discusses."*
- [X] T056 [US3] **DONE — four fences, eight problems, and the price is written down.** `session.itest.ts` at 3.17 adds blocks whose context is 9 of 14, 3 of 12 and **1 of 22** lines present in the chain: the region does not exist, 318 lines against ch16's **980**. Publishing that body would jump the reader 699 lines past what 3.3 showed them. So: session.itest.ts at 3.17 and 3.22, event.test.ts at 3.17, fanout.itest.ts at 3.17. **Those four amendments are no longer verified** and the files' end states go to the appendix; `session.itest.ts` stays verified for six chapters and declared for two. **Eight fence-pairs, so T090's population rises by 8 beyond phase 3's 22.** Original: Where no chapter honestly introduces a file, **convert AND record under FR-012**: turn that file's fences into excerpts — which is what takes the problem out of the count — and write down the reason, the measurement and what a real repair would cost. **Recording alone leaves the count non-zero**, because nothing reads `gaps.md` and the checker cannot tell a documented problem from any other. The count improving is not the test; whether a reader can still follow the chapter is. **Record how many fence PAIRS this converts** — the unit is the pair, so each one moves T090's population by **two** — because T090 asserts the declared-`(excerpt)` population and would otherwise fail for this task's reason. **T044 now offers this route as a design rather than a fallback**, so what reaches this task is a file for which all three designs were costed and none was honest, not a file nobody decided about.
- [X] T057 **DONE — `pnpm lint` exit 0, `pnpm build` exit 0.** Original: Run `pnpm build`; commit phase 5.
...
- [X] T058 [US1] **DONE — 29 files, 3,471 lines, and the total went UP.** Against the opening's 25 and 2,356, because ten files could not be compared before: `session.itest.ts` at **1,322** is the largest in the feature, `event.test.ts` 406, `turbo.json` and `harness.ts` 7 each. `vitest.coverage.config.mts` fell 866 → 743 and `eslint.config.mjs` 539 → **0**, both closed by phase 4. Original: Re-measure all 25 divergences **after phases 4 and 5**, because a divergence is whatever is left once every hunk has applied. The two largest — `vitest.coverage.config.mts` at **866** differing lines and `eslint.config.mjs` at **539** — were measured against a chain state those phases change, so **their real size is not yet known** and the plan says so rather than guessing.
...
- [X] T059 [US1] **DONE — 743 lines, in the appendix.** Chapter 4.9's prose-only note about this file is superseded: phase 4 repaired the chain so it can take a hunk now. Original: `vitest.coverage.config.mts` — append in `fences/post-series.md`. Chapter 4.9 could not publish its own two-line edit to this file for exactly this reason and described it in prose instead; that prose is replaced by the hunk here.
- [X] T060 [US1] **ALREADY CLOSED IN PHASE 4.** Regenerating the redundant appendix hunk brought the file to the tree exactly; 047-1 is closed. Original: `eslint.config.mjs` — append in `fences/post-series.md`. 206 chain lines against 451, **539 differing**.
- [X] T061 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `packages/protocol/src/codes.ts` — 156 differing lines.
- [X] T062 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/isolation/targets.ts` — 118 differing lines.
- [X] T063 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/auth/credential.guard.ts` — 101 differing lines.
- [X] T064 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/auth/credentials.itest.ts` — 85 differing lines.
- [X] T065 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `packages/protocol/src/codes.test.ts` — 78 differing lines.
- [X] T066 [P] [US1] **DONE, AND THE RECORDED DEFECT WAS IN THE WRONG PLACE.** FR-016 says *"the published fence carries `END $;`"*. It does not — it carries `END $$;`, and **the checker produced the `$;`**: `String.prototype.replace` reads `$$` in a string replacement as an escaped `$`. Two features filed an instrument bug against the prose. Fixed in the applier; the file needed a 19-line append, not the 56 the opening measured. Original: `packages/test-harness/src/sentinel.sql` — 56 differing lines. **And record separately that the published fence carries `END $;` where the repository has `END $$;`** (FR-016): a reader copying that listing gets a syntax error, and the count never said so.
- [X] T067 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/app.module.ts` — 53 differing lines.
- [X] T068 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `packages/test-harness/src/bound-port.test.ts` — 52 differing lines.
- [X] T069 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/gateway/src/presence.itest.ts` — 48 differing lines.
- [X] T070 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/outbox/event.ts` — 37 differing lines.
- [X] T071 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/gateway/src/main.test.ts` — 35 differing lines.
- [X] T072 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/internal/dispatch.controller.ts` — 20 differing lines.
- [X] T073 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/internal/usage.controller.ts` — 19 differing lines.
- [X] T074 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/isolation/targets.itest.ts` — 17 differing lines.
- [X] T075 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/isolation/fixtures.ts` — 17 differing lines.
- [X] T076 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `packages/test-harness/src/sentinel.ts` — 17 differing lines.
- [X] T077 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/auth/authenticate.middleware.ts` — 15 differing lines.
- [X] T078 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/webhooks/test-event.itest.ts` — 7 differing lines.
- [X] T079 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/src/db/catalogue.ts` — 6 differing lines.
- [X] T080 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `package.json` — 6 differing lines, and it is `test:integration` at line 15, which chapter 4.9 pointed at `scripts/integration-gate.mjs`.
- [X] T081 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/gateway/src/typing.itest.ts` — 4 differing lines, from chapter 4.9's presence-payload repair.
- [X] T082 [P] [US1] **DONE — appended in `fences/post-series.md`, re-measured after phases 4 and 5.** Original: `services/api/package.json` — 3 differing lines.
- [X] T083 [P] [US1] **DONE — one line, `corpus.json`, the pure append.** The only one of the 29 whose chain state is a strict prefix of its file, exactly as analysis pass 6 measured. Original: `.gitignore` — **one differing line, and it is a pure append: the cheapest target in the phase.** Measured: the chain state is **7 lines**, the tree holds **8**, the state is a **strict prefix**, and the line the tree has is `corpus.json` — a file the corpus scripts write and no chapter teaches, so `fences/post-series.md` is its home for the reason the appendix exists. Still diff the dumped state against the tree with `diff` and `cat -A` before writing, because whitespace is the one difference a `@@` hunk cannot always express.
...
- [X] T084 [US1] **DONE — per file in `baseline.txt`.** All 29 hunks generated at `-U6` with every pre-image unique on the first attempt, from `check:fences --dump` rather than a `git diff` against the tree. No file raised the total. Original: Re-measure after each file and record the total, APPLY, HEAD and `MIRROR` with the file that preceded each number, and the command that generated its hunk (FR-003, FR-011, SC-004, SC-009). A file that raises the count is reverted or finished before the next is started (FR-004).
- [X] T084a [US3] **DONE — zero of the 29 went into a chapter; all 29 are in `fences/post-series.md`.** So SC-007 has nothing to read in this phase: the appendix exists precisely so a change no chapter teaches is not put in front of a reader, and every one of these is that. Original: **Read the prose beside every hunk this phase appended in a chapter** and confirm it still describes the listing; record the count of chapters read (SC-007). Hunks that landed in `fences/post-series.md` are exempt by construction — the appendix exists so that a change no chapter teaches is not put in front of a reader, which is the checker's own header comment and the reason FR-007 prefers it. **So record the split: how many of the 25 went to the appendix and how many into a chapter**, because the second number is the only part of this phase a reader sees.
- [X] T085 **DONE — `pnpm lint` exit 0, `pnpm build` exit 0.** Original: Run `pnpm build`; commit phase 6.
...
- [X] T086 [US2] **DONE — the success line, verbatim: `282 fenced files replay onto relay-platform across 52 chapters (46 translated, fences mirrored, 2 retired, plus post-series amendments)`, EXIT 0.** 282 = 285 − 11 declared + 8 bodies; not 283, because the ninth file was declared under FR-012 rather than published. Original: Run `pnpm check:fences` on a clean tree and confirm **0 problems** (SC-001), recorded beside T001's opening. **Assert the success LINE, not the exit code**: `check-fence-chain: N fenced files replay onto relay-platform across M chapters (…)`, with the line expected whole: **283 fenced files across 52 chapters (46 translated, 2 retired, plus post-series amendments)**.
...
- [X] T087 [US2] **DONE — RUN RED.** One comment planted in `services/api/src/db/catalogue.ts` takes the run to `1 problem(s) — HEAD 1`, EXIT 1, and the detail names the file and the line. Restored; `relay-platform` reports 0 dirty files and the clean tree exits 0 again. Original: **Plant a regression and run it red** (SC-003): change one line in a fenced platform file with no chapter hunk, confirm the run exits non-zero and names that file, then restore. Run red rather than reasoned about — the same probe chapter 4.9 used on the reconciler.
- [X] T088 [US2] **DONE — APPLY 0 · HEAD 0 · MIRROR 0 · SYNTAX 0**, and each of the four classes reported separately at 0. Original: Confirm `MIRROR` is 0 (SC-004) and that the four classes are each 0 (SC-005), reported separately rather than as one total. **`MIRROR` at 0 is the only reading of it that means what it says** — see T006: at any other value, the chapters it names had no body comparison at all, so the number is a count of chapters skipped rather than of fences wrong.
- [X] T089 [US2] **DONE — `git diff --stat part4-ch9` is empty and `git status --short` reports 0.** Not one platform file was edited, which was the cheapest available repair for 25 of the original 110. Original: Confirm no file under `relay-platform/` was edited by this feature: `git diff --stat part4-ch9` in that repository is empty (SC-010). **A requirement, not an observation** — the cheapest repair for 25 of the 110 would have been to edit the platform.
- [X] T090 [US2] **DONE — untitled unchanged at 360; `(excerpt)` 222 → 252**, which is +22 from phase 3 and +8 from T056, checked from both sides. Per file: `fanout.itest.ts` 2 pre-existing + 1 declared, `session.itest.ts` 2 + 2, `event.test.ts` 0 + 1 — four English fences and four Vietnamese twins. `NOT_A_FILE` skips 254 including the `.naive.` pair. Nothing that names a real file became unverified without a recorded exception. Original: Re-measure the unread-fence population (SC-008) and confirm untitled is unchanged at 360 and declared `(excerpt)` has risen from **222 to 244, plus two per fence-pair T056 recorded** — **22** from phase 3, because the eleven prose titles are eleven English fences and eleven Vietnamese ones, and phase 3 declares both. **An assertion of "exactly 11" would fail for T056's reason rather than its own**, which is the defect class this project keeps finding. Nothing that names a real file may have become unverified **without a recorded exception**.
...
- [X] T091 [US2] **DONE — 110 is the highest number in every per-target table, and there was one excursion.** Phase 6's applier fix took the count **1 → 4**, exposing three APPLY problems that had been passing on a corrupted replacement; completed inside `sentinel.sql`'s own work rather than carried forward (FR-004). Original: Confirm the count never exceeded 110 at any measured point (SC-006), from `baseline.txt`'s per-target record. If it did, the excursion is reported with the file that caused it and the number it reached.
...
- [X] T092 [US2] **DONE — the five gates from `ci.yml:184-205`, each read by its counted success line.** `lint` 0, `build` 0, `check-docs-drift: all mirrored docs match their sources`, `check-revision-order: 17 revisions ascend`, `check:srs: 245 clause rows, 245 unique identifiers`, `check-figures: 281 figures … 3 export(s) unused`. The three figure notes predate this feature — `figTwoCounters` is in that `figures.ts` at `ba23f7b`. Original: Run the tutorial job's five other gates, **named from `ci.yml` rather than remembered** (`.github/workflows/ci.yml:184-205`): `pnpm lint`, `pnpm build`, `pnpm check:docs`, `pnpm check:srs`, `pnpm check:figures`. The sixth step is `check:fences`, which T086 covers.
  **`check:errors` IS NOT ONE OF THEM, AND THAT IS THE FINDING.** This task named it until analysis pass 7 opened the workflow: **no job in `ci.yml` runs `check:errors`** — not `tutorial`, not `platform`, not `outsider` — so it was the only reason this task carried *"build `relay-platform` first"*, minutes of work guarding a gate the job SC-002 is about never reaches. Run it anyway if you like, labelled as extra; **file the real finding in `gaps.md` (T095)**: one of `relay-tutorial`'s five `check:*` scripts is wired into no workflow step.
  **ASSERT EACH GATE'S SUCCESS LINE, NOT ITS EXIT CODE** — the same rule T086 applies to `check:fences`, for the same reason and four more times over. **Five of the seven gate scripts exit 0 when their corpus is absent**: `check-docs-drift.sh:36`, `check-srs-ids.sh:41`, `check-revision-order.mjs:34`, `check-fence-chain.sh:11`, `check-fence-chain.mjs:200`. Every one of them prints a line with a count in it when it really looked, so the counts are the evidence:
  ...
  The best statement of this rule in the repository is a comment in the gate nothing runs — `check-error-codes.mjs:33-35`, *"the difference between a skip somebody investigates and a skip somebody ignores."*
- [X] T093 **DONE.** Original: Commit phase 7.
...
- [X] T094 **DONE — ADR-29 in both documents, then `pnpm sync:docs` and the docs gates re-run after the edit.** Both mirrors carry it; `check:docs` and `check:srs` green. It records the two changes as one decision about what a titled fence claims, the five rejected alternatives, and the applier fix as a bug rather than a decision. Original: Write **ADR-29** in `docs/05-sad.md` and `docs/06-adr-deep-dives.md`: the `--dump` flag and the `(excerpt)` declarations as one decision about what a fence claims, with the rejected alternatives — teaching the checker that `lang=text` is never a file, retitling to the real path, removing the titles — and a reversal condition. Constitution VII, and the plan flagged it as owed.
  **THEN `pnpm sync:docs`, AND RUN `check:docs` AND `check:srs` AGAIN** — both documents this task edits are in `check-docs-drift.sh`'s explicit DOCS list and are compared byte-for-byte against their mirrors in `relay-tutorial/content/docs/`. **The mirror is not updated by editing `docs/`**, and until analysis pass 7 the word `sync:docs` did not appear anywhere in this feature. T092 ran those gates in the **previous phase**, so as the tasks stood the only thing that would catch a stale mirror or a malformed ADR was CI after the push — and T102 is told to record a failure rather than repair it. **The docs gates have to run after the last `docs/` edit**, which is this task and not T092. Chapter 4.9 paid for the same shape with an SRS revision row inserted out of order, found by running the checker rather than by reading the file.
- [X] T095 **DONE — `gaps.md`, 12 entries: 7 new, 4 closed (054-1, 050-4, 047-1, 048-3), 1 corrected (FR-016 → 055-1), 1 carried unchanged (043-1).** 050-3 gains the numbers its prose described. Original: Write `specs/055-fence-chain-repair/gaps.md`. **Re-measure the carried items this feature touches** rather than copying them: 043-1 (untitled fences, re-measured at 360 of 2,109), 047-1 (`eslint.config.mjs` could not take a fence), 048-3 (`vitest.coverage.config.mts` diverged at line 29), 050-3 (the vi chain is never compared to the tree — **carry the measurement, not the sentence**: the appendix loop mutates `en.state` only and `const vi = replay("vi", …)` runs at `check-fence-chain.mjs:300`, after both the appendix and the HEAD comparison, so the two chains' end states differ by **9 paths** and `turbo.json` ends at **en 75 · vi 62**. That is the mechanism the entry describes in prose, and this feature can state it in numbers because phase 2 built the instrument that reads it), 050-4 (the 11 prose titles), 054-1 (the red workflow, closed by this feature).
  **And open two new ones, both measured by analysis pass 7.** (a) **`check:errors` has no runner**: it is a script in `relay-tutorial/package.json` and a step in no job in `ci.yml`, which is 051's *"referenced by no script, service or config"* and 054's *"the daily job has no runner of any kind"* arriving in the tutorial's own gates. (b) **Five of the seven gate scripts exit 0 when their corpus is absent** — `check-docs-drift.sh:36`, `check-srs-ids.sh:41`, `check-revision-order.mjs:34`, `check-fence-chain.sh:11`, `check-fence-chain.mjs:200` — so the failure mode SC-001 names for one gate is a class of five. This feature found it three times before counting it, which is the project's own rule about classes: **when the count of a class keeps growing, stop counting failures and sweep.**
- [X] T096 **DONE — in `gaps.md` and in `baseline.txt`.** Not readable chapters, not right listings, not the **614 fences outside every gate**, not a Vietnamese chain compared to anything, and the success line's 52 is pages the walker found. Original: In `gaps.md`, record **what zero does not mean**: not that the chapters are readable, not that the listings are pedagogically right, not that the 360 untitled fences mean anything. One property — every titled fence replays onto the repository.
...
- [X] T097 **DONE — `gaps.md` 055-2.** 18 shadows down; ten invisible divergences up; four caused entirely by their own failing hunk. 25 − 4 + 2 + 6 = 29. Original: In `gaps.md`, record the cascade arithmetic (FR-015): how many of the 110 were single defects and how many were shadows of an earlier failure in the same file, so the next reader of a fence-chain number knows what it counts.
- [X] T098 **DONE — `traceability.md`, with four discharged weaker than their words**: SC-007 covers the English chapters only, FR-008's 30 new declarations include four that are a loss, FR-016 is withdrawn rather than met, and FR-002 permitted one unauthorised change — the applier fix. Original: Write `specs/055-fence-chain-repair/traceability.md` mapping every FR and SC to the tasks and artifacts that discharged it, and **record any discharged in a weaker form than their words suggest**.
- [X] T099 **DONE.** Original: Update `CLAUDE.md`'s `<!-- SPECKIT -->` block for the close, including every task premise this feature falsified by running it. **Its inventory table already carries one correction from analysis pass 8** — `2,381 differing lines` was one too high per file and now reads **2,356, 1 append and 24 interior** — so check the rest of that table against the close-out measurement rather than editing around it.
- [X] T100 **DONE — DECIDED: NO NEW MECHANISM.** The checker already exits non-zero above 0 and `ci.yml` runs it on every push; that is the guard, and it starts working the moment the count is 0. **A gate that is always red guards nothing** — which is what nine chapters demonstrated rather than a missing tool. What failed was the convention: 046 reported 110 against an inherited 109, attributed it to the wrong repository, and every chapter since compared a total to a total. **The change is to report the absolute number, not the delta.** Nothing to build. Original: Consider whether zero should be **guarded** rather than merely reached — the checker already exits non-zero above 0, so the open question is what holds the number between chapters. Adjacent to `gaps.md` 048-5 and 054-4; record the decision either way rather than leaving the next chapter to discover it.
