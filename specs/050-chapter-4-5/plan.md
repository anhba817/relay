...
| **III, the 0.1% clause** | *"Metered totals MUST reconcile against operational counts to within 0.1%, verified by a daily job that alerts on breach."* FR-009 is a reconciliation. | **PASS, and the bound is not this chapter's.** The daily job is movement IV and is named in Out of Scope; this chapter builds the second counter it will read, and publishes the duration-against-buckets gap as a number rather than against a threshold. **048-1 is still open on that 0.1%** — 047 measured 0.49% on the boundary day and 0.51% on `uniq` at 70,000 distinct — so a chapter asserting the bound here would be asserting something two measurements already contradict. This row was absent until analysis pass 10. |
...
| **VII, ADR immutability** | *"ADRs are immutable once accepted; superseding requires a new ADR."* FR-016 changes ADR-07's record. | **OPEN, and FR-016a closes it deliberately.** ADR-07 carries both forms already: in-place amendments dated 2026-08-04 and 2026-09-03, and a status line reading *"extended by ADR-20 … and by ADR-22"*. Precedent therefore does not decide it and the constitution names only one. Governance requires the conflict *"resolved explicitly by amendment rather than silent divergence"*, so the defect was the missing sentence rather than either choice. **This row was absent until analysis pass 10**, while the VII row below cited the principle for scope. |
