...
- [ ] T005 [P] Quote ADR-07's v1.1 amendment into `specs/050-chapter-4-5/baseline.txt` **from `docs/05-sad.md`**, because that is where v1.1 is. **And open `docs/12` §7.2 rather than avoiding it**: an earlier version of this task called its *"clean mapping — gateway to Redis"* a paraphrase and *"a phrase that appears nowhere in the SAD"* — true of `docs/05` and misleading, because it is **verbatim in `docs/06-adr-deep-dives.md`**, which is where ADR-07's argument lives. §7.2 is quoting, and it is this chapter's own open question (T085a). Quote all three into the baseline — the SAD's v1.1, the deep dive's selection argument, and §7.2's brief — so the amendment is written against the record rather than against one excerpt of it. 049 did find five stale cross-references in §3; that is not a reason to distrust §7.
