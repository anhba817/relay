...
| column | producer | when |
|---|---|---|
| `environment_id`, `channel_id`, `user_id`, `ts` | the corpus load, then the ingester | this chapter, then movement II |
| `event` | `'created'` only | `edited`/`deleted` have Part 3 producers and no analytical writer |
| `text_length` | `length(messages.text)` | this chapter |
| `attachment_count` | `length(messages.attachments)` — 0 for every corpus row | this chapter |
