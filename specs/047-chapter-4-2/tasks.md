...
- [ ] T020 [US1] Create `relay-platform/scripts/scale/load-analytics.mjs`: one `INSERT INTO message_events SELECT … FROM postgresql('postgres:5432', …)` joining `messages` to `channels` — the join across two table functions was checked and returns 303,885. **Three expressions are not the obvious ones**, and `data-model.md` §1 carries the measurements:
