...
- [ ] T052 **Publish TWO hunked ```diff fences** in `.../page.mdx` — `compose.yaml`, following the four Part 3 chapters that already amend that file, **and `eslint.config.mjs`**, which Phase 2 had to amend so `analytics/**/*.mjs` gets Node globals: without it `pnpm lint` fails with 13 `no-undef` errors on `fetch`, `process`, `console` and `Buffer`. **`eslint.config.mjs` is amended in `fences/post-series.md` too**, so check the appendix hunk still applies after this one and do not assume it does. **Do not regenerate chapter 1.2's whole-body fence** — that satisfies the per-chapter checker and unanchors ninety-two downstream hunks.
