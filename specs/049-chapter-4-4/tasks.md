...
- [ ] T050 [US1] Take the value from **`refusal.operation`**, the single operation that tripped — not from `operationsFor`'s array. The limiter already narrows it and already names it to the customer: `rate-limit.middleware.ts:212` reads `refusal.operation === "send" ? "messages" : "requests"`.
