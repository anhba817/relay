...
- [ ] T086 Write `specs/039-chapter-3-21/check-prose.py` **before correcting anything**, on chapter 3.20's model: one fragment per contradicted claim **per locale**, matched against whitespace-collapsed text. **Four claims, eight fragments** — research R8 has the list, and **one of them is in `docs/08-error-reference.md` rather than in a chapter.** The checker's paths must span both trees: `relay-tutorial/app/(en)` and `(vi)` for chapter narrative, and `docs/` for the product's own documents. R8's first version searched only the tutorial, which is how a customer-facing claim survived two analysis passes. **Run it and watch all eight fail.** An instrument written after the corrections it checks can only ever be green. Chapter 3.20's caught its own author's wrong chapter slug on the first run, which is why the missing-file arm exists.
