...
- [ ] T005a Create `packages/test-harness/package.json` — a new workspace package (`packages/*` is already a `pnpm-workspace.yaml` glob) declaring its own `pg`. **A package rather than a directory inside `services/api`**, because five configs across four packages import it and a gateway test lane reaching into another service's `src/` is a worse precedent than a shared package, even in test code
- [ ] T005b **Record the second-language decision before writing any PL/pgSQL** — a note in `docs/06-adr-deep-dives.md`, not a numbered ADR, with research R6 and R10 as the evidence constitution VII's escape clause asks for. **Moved here from close-out**: an ADR is immutable once accepted, and deciding a constitution question after the code that depends on it is the wrong order
...
- [ ] T044 **Confirm T005b's note still describes what shipped.** The decision itself moved to the setup phase, because an ADR is immutable once accepted and a constitution question settled after the code is settled too late. This task only checks the note against the delivered design
...
**And one that has to happen before code**: T005b, the second-language note. An
