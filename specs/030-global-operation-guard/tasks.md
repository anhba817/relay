...
- [ ] T005a **Record the second-language decision before writing any PL/pgSQL** — a note in `docs/06-adr-deep-dives.md`, not a numbered ADR, with research R6 and R10 as the evidence constitution VII's escape clause asks for. **Moved here from close-out**: an ADR is immutable once accepted, and deciding a constitution question after the code that depends on it is the wrong order
- [ ] T006 Create `services/api/src/testing/sentinel.ts`: **one sentinel per test file**, its ids derived from the file path so they are stable across runs and unique across files, every row named `__sentinel__:<file>` (data-model, FR-023). **Not one shared sentinel** — files execute in parallel, so a shared one means one file's planting deletes rows another file is mid-test against (research R12)
...
- [ ] T009 Write the planting function in `services/api/src/testing/sentinel.ts` — **delete this file's sentinel rows then re-insert**, rather than `ON CONFLICT`. The environment id makes the delete exact, and the seeder must not become the accumulation it exists to simulate (FR-003)
- [ ] T009b **Plant through a dedicated `pg.Client` that never enters the suite's pool**, created with the exemption in its options and closed before the first test (FR-024). Deleting a sentinel row is exactly what the trigger forbids, so the seeder needs the exemption — and a connection carrying it that a test later reused would leave that test unguarded. Circular until the connection is separate (research R12)
...
- [ ] T010 [US2] Write `services/api/src/testing/sentinel.sql`: the `__sentinel_guard()` PL/pgSQL function and one `BEFORE UPDATE OR DELETE` row trigger per guarded table, firing when `OLD.environment_id` is **in `__sentinel_environments`**. Guarded tables are those carrying `environment_id`: `webhook_endpoints`, `webhook_deliveries`, `webhook_disable_notifications`, `channels`, `users`
...
- [ ] T012 [US2] Read the exemption in `services/api/src/testing/sentinel.sql` with `current_setting('relay.allow_global', true)`, so a connection that never carried it gets null and null is refusal
- [ ] T012a [US2] **Carry the exemption as a connection option, not a `SET` statement** (FR-020): the setup hook rewrites `process.env.DATABASE_URL` with `?options=-c%20relay.allow_global%3Don` for its own worker, before the suite calls `createPool()`. **Measured**: a `SET` issued through a pool of three landed on two of five checkouts — `["on",null,null,"on",null]` — so the naive version produces an exempt suite that fails two times in five (research R10). The connection string rather than the pool's config object, because that needs no change to `createPool()`
...
- [ ] T016c [US2] **Fail a non-exempt file at startup if it has a relay enabled** (FR-025). Both relays catch and log their own errors, so a refusal raised inside one is a log line and a green lane (research R13). Every suite that spawns an api child sets all four flags off today; that is a convention repeated in seven files, and this makes it checked
...
- [ ] T018 [US1] Plant the bait **per file, into that file's own sentinel**, in a `beforeAll` from `services/api/src/testing/setup.ts`. Research R2 measured three of the four baits eaten in a single lane pass, so a one-shot `globalSetup` seeder protects whichever suite runs first and nothing after it
...
- [ ] T044 **Confirm T005a's note still describes what shipped.** The decision itself moved to the foundational phase, because an ADR is immutable once accepted and a constitution question settled after the code is settled too late. This task only checks the note against the delivered design
...
**And one that has to happen before code**: T005a, the second-language note. An
