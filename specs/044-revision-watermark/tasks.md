...
- [ ] T003 Record in `baseline.txt` which changed files are fenced, from `grep -rl 'title="<path>"'` in `relay-tutorial`: `frames.ts` 6, `internal.ts` 12, `schema.ts` 17, `repository.ts` 24, `resume.ts` 2, `session.ts` 16, `repository.itest.ts` 10, `frames.test.ts` 6 — and **`session.itest.ts` excerpt-only, which no gate can see.**
...
- [ ] T009 Change `channelsForUser` in `repository.ts` to return `{ channel_id, revision_sequence }` with a join to `channels`, and map to ids in `relay-platform/services/api/src/internal/memberships.controller.ts`. **It has two callers** — the session route and the memberships route — and giving each its own query is the two-lists-that-must-agree defect `gaps.md` 3.23-4 records about `targets.ts`. One query, one source. (FR-014)
...
- [ ] T011 [US1] Add `channel_revisions: z.record(z.string(), z.number().int().nonnegative()).default({})` to `internalSessionResponseSchema` in `relay-platform/packages/protocol/src/internal.ts`. **`.default({})` follows `banned`'s precedent in this same schema** — an api built before this feature still satisfies it during a rolling deploy, and the gateway then behaves as it does today. (FR-004, FR-014)
- [ ] T012 [US1] Fill it in `relay-platform/services/api/src/internal/session.controller.ts:129` from T009's rows. `channel_ids` keeps its shape: eleven chapters publish that field and widening it into objects would edit all of them for a field they do not read. (FR-004)
- [ ] T013 [US1] Add `revisionCountSchema` and the ack's `revisions` field to `relay-platform/packages/protocol/src/frames.ts`. **Not `cursorSchema`** — it is `.positive()` and every unrevised channel is **zero**, so reusing it makes an unrevised channel unrepresentable and collapses the two states FR-007 turns on (research R4). (FR-004)
...
- [ ] T015 [US1] Compare and fill the ack in `relay-platform/services/gateway/src/session.ts`. Report **every** channel the user belongs to, including zeros and channels the client asked nothing about — a client needing no repair still needs a baseline to store. A presented count higher than the platform's is treated as no repair and **must not refuse the connection**: refusing over a number the client supplied is a denial of service the client controls. (FR-006, FR-008, FR-009)
- [ ] T016 [US1] Amend **FR-007 in `specs/044-revision-watermark/spec.md`** to separate a first connection from a pre-upgrade client resuming, per research R3. FR-007 covers "no count" as a first connection; a client presenting a **cursor** and no counts is a different case and the common one for a deploy window. Read literally, FR-007 tells every un-upgraded client that every revised channel needs repair, on every reconnect, during the window when the whole fleet is reconnecting. **A requirement that has to be read generously is one the next person reads differently** — amend it rather than interpret it.
...
- [ ] T018 [US1] Append an amendment hunk to `relay-tutorial/fences/post-series.md` for each fenced file changed in this phase — `internal.ts`, `frames.ts`, `resume.ts`, `session.ts`, `frames.test.ts` — and re-run `check:fences` after each. **Regenerate at `-U6` if a pre-image matches twice.**
- [ ] T019 [US1] Verify against `quickstart.md` scenarios 1 through 5, including scenario 5's cursor-still-parses check with a channel id containing a colon if one can be produced.
...
- [ ] T020 [US2] Handle the channel joined during the absence in `relay-platform/services/gateway/src/session.ts`: the client held nothing in it, so it must not be reported as needing repair even though the platform's count exceeds the absent one. Its messages arrive by the ordinary replay. (US2 scenario 3)
...
- [ ] T022 [US2] Append amendment hunks for anything this phase changed, and re-run `check:fences`.
...
- [ ] T024 [US3] Amend **FR-016a and FR-016b** in `docs/04-srs.md` to name the signal, so the documented limit and the documented remedy sit together. Add revision **1.10** to Appendix D, appended below 1.9 — `check-revision-order.mjs` fails on a descent. (FR-013, SC-007)
- [ ] T025 [US3] Verify `quickstart.md` scenario 7, and re-run `check:docs`: `docs/04-srs.md` and `docs/05-sad.md` are both mirrored into `relay-tutorial/content/docs/`, so `pnpm sync:docs` must run or the drift check fails.
...
**US3 depends only on Phase 2.** The documentation describes the count, which exists after T005
through T010; it does not need the transport. It can ship before US1 if the counter is built
and the signal is not.

## Parallel opportunities

- **T028 and T029** are different files and different questions.
- Within US1, **T011/T012** (api side) and **T013/T014** (protocol and gateway parsing) touch
  different packages until T015 joins them.
