...
FOREIGN: set[tuple[str, str]] = {
    # EMPTIED WHEN THIS COPY WAS MADE. Chapter 3.23's copy arrived carrying thirty-one
    # pairs naming chapter 3.22's task ids, and its docstring said "chapter 3.21" — both
    # caught by running the thing, neither by reading it. Cleared on purpose here, and
...
def main() -> int:
    ...
    if tasks_p.exists() and spec_p.exists():
        ...
        for ident in re.findall(r"^- \*\*((?:FR|SC)-\d+[a-z]?)\*\*", spec_body, re.M):
            # A NON-CHAPTER FEATURE HAS NO SUFFIX (analysis pass 9): the `(3.24)` form
            # disambiguates ids across chapters, and `043-fix-review-findings` yields
            # CHAPTER "?.?", which matched nothing and read every requirement as untasked.
            cited = re.escape(ident) + (r"\b(?! \()" if CHAPTER == "?.?"
                                        else r" \(" + re.escape(CHAPTER) + r"\)")
    ...
    if tasks_p.exists() and spec_p.exists():
        ...
        seen: dict[str, int] = {}
        # `SRS FR-016a` NAMES ANOTHER DOCUMENT'S CLAUSE, and is not a claim about this spec.
        # Feature 044 is built on two SRS clauses and cites them throughout; without this the
        # checker reports every citation as an undeclared requirement. The prefix is required
        # rather than optional — a bare `FR-016a` here would still be a defect, because this
        # spec's own numbering reaches FR-015 and the next id it mints collides.
